...
function loadSystem3dCenterV131(){
  ...
  wrap.innerHTML=`<div class="eng-card v131-load-card"><div class="section-db-head"><div><h2>3D Building Load System — V1.45</h2><small>Advanced Member Loads • UDL + Point + Trapezoidal + Member Moment • solver-linked</small></div><button class="ml-close">×</button></div>
  ...
  <section><h3>Load Summary</h3><div id="v131Summary" class="v131-summary"></div><div class="engineering-note"><b>V1.45 scope:</b> Advanced 3D member loads are assigned inside <b>3D Loads</b> and feed the 3D frame solver through consistent equivalent nodal loads. Point/Trapezoidal forces use Global X/Y/Z direction; Member Moment uses Local 1/2/3 axis. Positions are measured from member end <b>i</b> as x/L.</div></section></div>
  ...
  q('v131Assign').onclick=()=>{
    ...
    for(const m of ms)memberPatternLoadsV131(m,p).push({...ld,source:'V1.45_ADVANCED_MEMBER_LOAD',story:memberStoryV131(m)});m3.activeLoadPattern=p;m3.results=null;refresh();toast(`V1.45 assigned ${type} to ${ms.length} 3D beam${ms.length===1?'':'s'}`)
  };
...
function rcBeamDesignCenterV141(){
  ...
  w.innerHTML=`<div style="width:min(1120px,96vw);max-height:93vh;background:#fff;border-radius:18px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 24px 70px #0005">
    <header style="padding:18px 20px;background:#173b68;color:#fff;display:flex;justify-content:space-between"><div>
      ...
      <div style="font-size:13px;opacity:.84">V1.45 • Advanced Member Loads</div></div>
