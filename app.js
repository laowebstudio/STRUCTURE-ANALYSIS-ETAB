...
function solve3DV128(){
 ...
 const free=[...Array(nd).keys()].filter(i=>!fixed.has(i)),Kff=free.map(i=>free.map(j=>K[i][j])),Ff=free.map(i=>F[i]),uf=solveLinear(Kff,Ff),U=Array(nd).fill(0);free.forEach((d,i)=>U[d]=uf[i]);const RF=matVec(K,U).map((v,i)=>v-F[i]);
 ...
 const memberForces=elements.map(e=>{const ug=e.dofs.map(d=>U[d]),ul=matVec(e.T,ug),fl=matVec(e.kl,ul).map((v,i)=>v-(e.feLocal?.[i]||0));return{id:e.m.id,i:e.m.i,j:e.m.j,local:fl}});m3.results={U,F,K,displacements,reactions,memberForces,loadPattern:pat};return m3.results
}
...
function initIntegrated3DV128(host){
 ...
 function drawForceDiagrams(){
  ...
  const all=m3.results.memberForces.flatMap(f=>[Number(f.local[q[0]])||0,-(Number(f.local[q[1]])||0)]),maxAbs=Math.max(1e-9,...all.map(Math.abs));
  ...
  for(const f of forceRows){
   const mm=m3.members.find(x=>x.id===f.id),a=mm&&m3.nodes.find(n=>n.id===mm.i),b=mm&&m3.nodes.find(n=>n.id===mm.j);if(!a||!b)continue;
   const A=proj(a,r.width,r.height),B=proj(b,r.width,r.height),mid={x:(a.x+b.x)/2,y:(a.y+b.y)/2,z:(a.z+b.z)/2};
   const axes=v128Axes(a,b),axis=q[4]==='ez'?axes[2]:axes[1],axisPt={x:mid.x+axis[0],y:mid.y+axis[1],z:mid.z+axis[2]},M=proj(mid,r.width,r.height),P=proj(axisPt,r.width,r.height),vx=P.x-M.x,vy=P.y-M.y,ll=Math.hypot(vx,vy)||1,nx=vx/ll,ny=vy/ll;
   const vi=Number(f.local[q[0]])||0,vj=-(Number(f.local[q[1]])||0),oi=amp*vi/maxAbs,oj=amp*vj/maxAbs,AI={x:A.x+nx*oi,y:A.y+ny*oi},BJ={x:B.x+nx*oj,y:B.y+ny*oj};
   cx.save();cx.fillStyle=q[2]+'28';cx.strokeStyle=q[2];cx.lineWidth=(focusTarget?.type==='member'&&focusTarget.id===f.id)?4:2;
   cx.beginPath();cx.moveTo(A.x,A.y);cx.lineTo(AI.x,AI.y);cx.lineTo(BJ.x,BJ.y);cx.lineTo(B.x,B.y);cx.closePath();cx.fill();cx.stroke();
   cx.setLineDash([3,3]);cx.lineWidth=1;cx.beginPath();cx.moveTo(A.x,A.y);cx.lineTo(AI.x,AI.y);cx.moveTo(B.x,B.y);cx.lineTo(BJ.x,BJ.y);cx.stroke();cx.setLineDash([]);
   if(showDiagramValues){cx.fillStyle=q[2];cx.font='bold 10.5px Arial';cx.fillText(vi.toFixed(2),AI.x+4,AI.y-4);cx.fillText(vj.toFixed(2),BJ.x+4,BJ.y-4)}cx.restore();
  }
 }
