...
function buildingCenterV111(){
 ...
 wrap.innerHTML=`<div class="eng-card building-card"><div class="section-db-head"><div><h2>Building Center — V1.15 Fix</h2><small>2D Grid, Story Manager, Typical Floor, Floor/Wall Loads and Building Summary</small></div><button class="ml-close" id="v11Close">×</button></div>
...
function modelingToolsV112(){const wrap=document.createElement('div');wrap.className='eng-dialog modeling-modal';wrap.innerHTML=`<div class="eng-card modeling-card"><div class="section-db-head"><div><h2>Drawing & Modeling Tools — V1.15 Fix</h2><small>Transform, copy and clean the selected structural model.</small></div><button class="ml-close" id="mtClose">×</button></div><div id="mtFeedback" class="modeling-feedback"><span class="feedback-dot"></span><div><b>Ready</b><small>Choose a selection or modeling command. Your last action will appear here.</small></div></div><div class="modeling-grid">
...
function loadCenterV115(){
 ...
 wrap.innerHTML=`<div class="eng-card load-center-card"><div class="section-db-head"><div><h2>⬇ Load Center — V1.15 Fix</h2><small>Automatic self weight, generated-load management and combination templates.</small></div><button class="ml-close" id="lc15Close">×</button></div>
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.15 Fix — Professional menu ready');
})();


// V1.15 Fix — Professional menu and quick toolbar command routing
