...
function designCenterV120(){
 const wrap=document.createElement('div');wrap.className='eng-dialog design-modal-v120';wrap.innerHTML=`<div class="eng-card design-card-v120"><div class="section-db-head"><div><h2>◆ Design Center — V1.25.1 Fix</h2><small>Steel Design V1.24 preserved • RC Beam Design Phase 1 • ACI CODE-318-25 Flexure</small></div><button class="ml-close" id="d120Close">×</button></div><div id="d120Body"></div></div>`;document.body.appendChild(wrap);wrap.querySelector('#d120Close').onclick=()=>wrap.remove();wrap.onclick=e=>{if(e.target===wrap)wrap.remove()};const body=wrap.querySelector('#d120Body');
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.25.1 Fix — RC Beam internal-moment demand transfer fixed • Steel V1.24 preserved');
})();
