...
function steelDesignV121(m){
 ...
 const status=axialRatio<=1?'PASS':'FAIL';
 return{applicable:true,q,status,A,L,rmin,slender,Fe,Fcr,phiT,phiC,phiAxial,axialRatio,phiMy,flexRatio,note:flexRatio==null?'Axial LRFD check':'Axial LRFD + flexure yield screening'};
}
...
function designCenterV120(){
 const wrap=document.createElement('div');wrap.className='eng-dialog design-modal-v120';wrap.innerHTML=`<div class="eng-card design-card-v120"><div class="section-db-head"><div><h2>◆ Design Center — V1.21</h2><small>Analysis → Steel Design Phase 1 • AISC 360-22 LRFD • Axial strength + flexure screening</small></div><button class="ml-close" id="d120Close">×</button></div><div id="d120Body"></div></div>`;document.body.appendChild(wrap);wrap.querySelector('#d120Close').onclick=()=>wrap.remove();wrap.onclick=e=>{if(e.target===wrap)wrap.remove()};const body=wrap.querySelector('#d120Body');
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.21 — Steel Design Phase 1 ready');
})();
