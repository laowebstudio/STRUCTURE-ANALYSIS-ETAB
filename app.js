...
drawMemberLoadsV131();drawMemberLocalAxes();drawForceDiagrams();drawAxisTriad();drawLegend()}
 ...
 host.querySelector('#v1471Physical').onclick=()=>{physicalMode=true;m3.showLoads=false;host.querySelector('#v1471Physical').classList.add('primary');host.querySelector('#v1471Analysis').classList.remove('primary');host.querySelector('#v128TopStatus').textContent='V1.47.1.3 • ACCURATE WEBGL PICKING • TRUE SOLID PHYSICAL MODEL • Beam + Column + Slab + Foundation';draw()};host.querySelector('#v1471Analysis').onclick=()=>{physicalMode=false;host.querySelector('#v1471Analysis').classList.add('primary');host.querySelector('#v1471Physical').classList.remove('primary');host.querySelector('#v128TopStatus').textContent='V1.47.1.3 • ANALYSIS MODEL • Centerlines + Loads + Results';draw()};
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.47.1.3 — Accurate WebGL Member Picking + Selection Mapping Fix');
})();
