...
// ===== V1.47.2.2 — Full model regeneration + result-reset integrity =====
function reset3DModelAndAnalysisV14722(m3,reason='FULL MODEL REGENERATION'){
  if(!m3)return;
  // Geometry / physical objects
  m3.nodes=[];m3.members=[];m3.slabs=[];m3.foundations=[];
  m3.nextNode=1;m3.nextMember=1;
  // Selection / locate state
  m3.selectedNodeId=null;m3.selectedMemberId=null;
  // Current solved state + every derived analysis/design cache that can reference old IDs.
  m3.results=null;
  m3.loadCaseResults={};
  m3.comboResults={};
  m3.envelopeV140=null;
  m3.wholeModelDesignEnvelopeV146=null;
  m3.slabTransferSummary=null;
  m3.activeResultType=null;m3.activeResultName=null;
  m3.lastSolvedRevisionV1451=null;m3.lastSolvedPatternV1451=null;m3.lastSolvedAtV1451=null;
  if(m3.rcBeamDesignV141) m3.rcBeamDesignV141.results=[];
  // A regeneration is a model revision even if loads are unchanged.
  m3.analysisRevisionV1451=(Number(m3.analysisRevisionV1451)||0)+1;
  m3.analysisInvalidationReasonV1451=reason;
  m3.regenerationRevisionV14722=(Number(m3.regenerationRevisionV14722)||0)+1;
  m3.lastRegeneratedAtV14722=Date.now();
}
function current3DModelIntegrityV14722(m3){
  const nodeIds=new Set((m3.nodes||[]).map(n=>Number(n.id))),memberIds=new Set((m3.members||[]).map(m=>Number(m.id)));
  const badMembers=(m3.members||[]).filter(m=>!nodeIds.has(Number(m.i))||!nodeIds.has(Number(m.j)));
  const badSlabs=(m3.slabs||[]).filter(s=>(s.nodes||[]).some(id=>!nodeIds.has(Number(id))));
  const badFootings=(m3.foundations||[]).filter(f=>!nodeIds.has(Number(f.nodeId)));
  return {ok:!badMembers.length&&!badSlabs.length&&!badFootings.length,nodeIds,memberIds,badMembers,badSlabs,badFootings};
}
...
function building3dCenterV130(){
  ...
  q('v130Generate').onclick=()=>{try{
    ...
    if(q('v130Replace').checked)reset3DModelAndAnalysisV14722(m3,'REPLACE CURRENT 3D GEOMETRY');else {m3.results=null;invalidate3DDesignDerivedV1451('3D GEOMETRY APPENDED')}
...
function solve3DV128(){
 const integrityV14722=current3DModelIntegrityV14722(state.model3d||{});if(!integrityV14722.ok)throw new Error('Current 3D model failed integrity validation before solve. Regenerate the building.');
...
drawMemberLoadsV131();drawMemberLocalAxes();drawForceDiagrams();drawAxisTriad();drawLegend()}
 ...
 integrated3dRefreshV128=(doFit=false)=>{if(!m3.results){host.querySelector('#v128SolveStatus').textContent='Not analyzed • current model changed';host.querySelector('#v128ShowResults').disabled=true;host.querySelector('#v128ModalStatus').textContent='Not analyzed';host.querySelector('#v128ResultsModal').hidden=true;focusTarget=null;host.querySelector('#v128LocateBar').hidden=true;}if(doFit)fit();else draw();renderTab()};
