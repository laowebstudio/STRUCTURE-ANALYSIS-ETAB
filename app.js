...
function loadSystem3dCenterV131(){
  ...
  wrap.innerHTML=`<div class="eng-card v131-load-card"><div class="section-db-head"><div><h2>3D Building Load System — V1.45.2</h2><small>Advanced Member Loads • UDL + Point + Trapezoidal + Member Moment • solver-linked</small></div><button class="ml-close">×</button></div>
  ...
  <section><h3>Load Summary</h3><div id="v131Summary" class="v131-summary"></div><div class="engineering-note"><b>V1.45.2 scope:</b> Advanced 3D member loads are assigned inside <b>3D Loads</b> and feed the 3D frame solver through consistent equivalent nodal loads. Point/Trapezoidal forces use Global X/Y/Z direction; Member Moment uses Local 1/2/3 axis. Positions are measured from member end <b>i</b> as x/L.</div></section></div>
...
function envelopeV1452Live(){
  const m3=ensure3DLoadSystemV131();
  const combos=ensureLoadCombosV139();
  const candidates=[];
  // Keep the normal governing combinations.
  for(const c of combos){
    const r=solveLoadCombinationV139(c);
    candidates.push({name:c.name,result:r});
  }
  // V1.45.2: also solve the currently active pattern directly. This is the exact
  // result source used by the 3D workspace after Analyze 3D and guarantees that
  // Advanced Member Loads (Point/Trap/Moment) participate in RC demand even if a
  // legacy combination/cache path omits their fixed-end contribution.
  const original=m3.activeLoadPattern||'DL';
  let live=null;
  try{
    m3.activeLoadPattern=original;
    live=solve3DV128();
    if(live) candidates.push({name:`LIVE:${original}`,result:live});
  } finally { m3.activeLoadPattern=original; }
  ...
  for(const x of candidates){
    for(const f of (x.result?.memberForces||[])){
      if(!map.has(f.id)) map.set(f.id,{id:f.id,i:f.i,j:f.j,v:Array.from({length:12},()=>({max:-Infinity,min:Infinity,maxCombo:'',minCombo:''}))});
      (f.local||[]).forEach((n,k)=>{
        n=Number(n)||0; const q=map.get(f.id).v[k];
        if(n>q.max){q.max=n;q.maxCombo=x.name}
        if(n<q.min){q.min=n;q.minCombo=x.name}
      });
    }
  }
  const env={combos:candidates.map(x=>x.name),members:[...map.values()],livePattern:original,source:'V1.45.2 LIVE SOLVER + COMBINATIONS'};
  m3.envelopeV140=env;
  m3.rcLiveEnvelopeV1452={pattern:original,at:Date.now(),revision:Number(m3.lastSolvedRevisionV1451)||0};
  return env;
}

function rcBeamDesignV141(){
  const m3=ensure3DLoadSystemV131();
  // V1.45.1 synchronization fix: never design from a cached pre-load-change envelope.
  // Re-solve the defined load combinations so RC demand comes from the same current
  // loads that the 3D solver/diagrams use.
  const env=envelopeV1452Live();
...
function rcBeamRebar3DViewerV142(d){
  ...
  modal.innerHTML=`<div style="display:flex;align-items:center;justify-content:space-between;color:#fff;gap:12px;flex-wrap:wrap"><div><b style="font-size:20px">3D RC Rebar Viewer — M${d.id}</b><div style="font-size:12px;opacity:.78">V1.45.2 • Rebar Viewer Live Solver Results Fix</div></div><div style="display:flex;gap:7px;align-items:center;flex-wrap:wrap"><button id="v142fit">Fit</button><button id="v142iso">Isometric</button><button id="v142side">Side</button><button id="v142top">Top</button><button id="v142endi">End-i</button><button id="v142endj">End-j</button><label style="font-size:12px"><input id="v142conc" type="checkbox" checked> Concrete</label><label style="font-size:12px"><input id="v142st" type="checkbox" checked> Stirrups</label><label style="font-size:12px"><input id="v142bars" type="checkbox" checked> Bottom bars</label><label style="font-size:12px"><input id="v143topbars" type="checkbox" checked> Top bars</label><button id="v142close">Close</button></div></div><div style="position:relative;flex:1;min-height:360px;background:#e8eef5;border-radius:8px;overflow:hidden"><canvas id="v142canvas" style="width:100%;height:100%;display:block;touch-action:none"></canvas><div id="v142info" style="position:absolute;left:12px;top:12px;background:rgba(255,255,255,.94);padding:10px 12px;border-radius:8px;box-shadow:0 2px 8px #0002;font:12px Arial;line-height:1.55;color:#0f172a;max-width:360px"></div><div style="position:absolute;right:12px;bottom:10px;background:rgba(15,23,42,.86);color:white;padding:7px 10px;border-radius:7px;font:11px Arial">Drag: Rotate • Wheel: Zoom • V1.45.2 live-solver zoning viewer</div></div>`;
  document.body.appendChild(modal);
...
function rcBeamDesignCenterV141(){
  ...
  w.innerHTML=`<div style="width:min(1120px,96vw);max-height:93vh;background:#fff;border-radius:18px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 24px 70px #0005">
    <header style="padding:18px 20px;background:#173b68;color:#fff;display:flex;justify-content:space-between"><div>
      ...
      <div style="font-size:13px;opacity:.84">V1.45.2 • Rebar Viewer Live Solver Results Fix</div></div>
  ...
  w.querySelector('#v141Apply').onclick=()=>{
    ...
    designs=rcBeamDesignV141();w.querySelector('#v141tbody').innerHTML=rows();bindDetails();toast('V1.45.2 recalculated • Live solver → RC demand → Rebar Viewer synchronized');
  };
}
