...
function initIntegrated3DV128(host){
 state.model3d ||= {nodes:[],members:[],nextNode:1,nextMember:1,view:{yaw:-35,pitch:24,scale:34}};state.model3d.view ||= {yaw:-35,pitch:24,scale:34};const m3=state.model3d,c=host.querySelector('#v128Canvas'),cx=c.getContext('2d');let drag=null,view='model',tab='summary',focusTarget=null;m3.view.cx??=0;m3.view.cy??=0;m3.view.cz??=0;
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.28.5 — Interactive 3D Results Viewer • calculation engines preserved');
})();
