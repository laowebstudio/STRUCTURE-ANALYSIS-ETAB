...
function building3dCenterV130(){
  ...
  wrap.innerHTML=`<div class="eng-card v130-building-card"><div class="section-db-head"><div><h2>3D Building Generator — V1.30</h2><small>Independent 3D workspace • generates only state.model3d • 2D model is not modified</small></div><button class="ml-close">×</button></div>
  ...
  q('v130Generate').onclick=()=>{try{
    ...
    m3.nextNode=nid;m3.nextMember=mid;m3.building={stories,baysX:bx,baysY:by,storyHeights:hs.join(','),bayWidthsX:wx.join(','),bayWidthsY:wy.join(','),fixedBase:fixed};
    m3.selectedNodeId=m3.nodes[0]?.id||null;m3.selectedMemberId=null;
