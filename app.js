...
const state = {nodes:[],members:[],materials:JSON.parse(JSON.stringify(defaultMaterials)),sections:JSON.parse(JSON.stringify(defaultSections)),loadCases:JSON.parse(JSON.stringify(defaultLoadCases)),loadCombinations:JSON.parse(JSON.stringify(defaultLoadCombinations)),activeLoadCase:'DL',activeAnalysis:'CASE:DL', tool:'select', selected:null, memberStart:null, nextNode:1,nextMember:1, view:{scale:55,ox:120,oy:500}, dragging:null, panning:null, hover:null, results:null, resultTab:'summary', diagramScale:1, autoDiagramScale:true, showLabels:true,showLoadLabels:true,resultsByAnalysis:new Map(),multiSelectedMemberIds:new Set(),boxSelect:null,building:{stories:0,bays:0,storyHeights:[],bayWidths:[],levels:[],grids:[]},layers:{members:true,nodes:true,loads:true,supports:true,labels:true}};
...
async function cloudDialog(){
 ...
 wrap.innerHTML='<div class="cloud-card"><h2>☁ Supabase Cloud — V1.17</h2><div id="cloudBody">ກຳລັງໂຫຼດ...</div><button id="cloudClose">ປິດ</button></div>';
...
function buildingCenterV111(){
 ...
 wrap.innerHTML=`<div class="eng-card building-card"><div class="section-db-head"><div><h2>Building Center — V1.17</h2><small>2D Grid, Story Manager, Typical Floor, Floor/Wall Loads and Building Summary</small></div><button class="ml-close" id="v11Close">×</button></div>
...
function modelingToolsV112(){const wrap=document.createElement('div');wrap.className='eng-dialog modeling-modal';wrap.innerHTML=`<div class="eng-card modeling-card"><div class="section-db-head"><div><h2>Drawing & Modeling Tools — V1.17</h2><small>Transform, copy and clean the selected structural model.</small></div><button class="ml-close" id="mtClose">×</button></div><div id="mtFeedback" class="modeling-feedback"><span class="feedback-dot"></span><div><b>Ready</b><small>Choose a selection or modeling command. Your last action will appear here.</small></div></div><div class="modeling-grid">
...
function loadCenterV115(){
 ...
 wrap.innerHTML=`<div class="eng-card load-center-card"><div class="section-db-head"><div><h2>⬇ Load Center — V1.17</h2><small>Automatic self weight, generated-load management and combination templates.</small></div><button class="ml-close" id="lc15Close">×</button></div>
...
$('checkModelBtn').onclick=modelCheckDialog;$('checkModelSideBtn').onclick=modelCheckDialog;$('envelopeBtn').onclick=resultEnvelopeDialogV116;$('envelopeSideBtn').onclick=resultEnvelopeDialogV116;$('analyzeBtn').onclick=analyze;$('csvBtn').onclick=exportCSV;$('clearResultsBtn').onclick=clearResults;$('cloudBtn').onclick=cloudDialog;$('viewResult').onchange=e=>setResultView(e.target.value);$('autoScaleToggle').onchange=e=>{state.autoDiagramScale=e.target.checked;syncScaleUI();updateDiagramLegend($('viewResult').value);render()};$('diagramScale').oninput=e=>{state.diagramScale=Math.max(.2,Math.min(10,Number(e.target.value)||1));updateDiagramLegend($('viewResult').value);render()};$('scaleDownBtn').onclick=()=>{state.autoDiagramScale=false;state.diagramScale=Math.max(.2,state.diagramScale-.5);syncScaleUI();updateDiagramLegend($('viewResult').value);render()};$('scaleResetBtn').onclick=()=>{state.autoDiagramScale=false;state.diagramScale=1;syncScaleUI();updateDiagramLegend($('viewResult').value);render()};$('scaleUpBtn').onclick=()=>{state.autoDiagramScale=false;state.diagramScale=Math.min(10,state.diagramScale+.5);syncScaleUI();updateDiagramLegend($('viewResult').value);render()};$('labelToggle').onchange=e=>{state.showLabels=e.target.checked;updateDiagramLegend($('viewResult').value);render()};if($('loadLabelToggle'))$('loadLabelToggle').onchange=e=>{state.showLoadLabels=e.target.checked;render()};document.querySelectorAll('.result-mode').forEach(b=>b.onclick=()=>setResultView(b.dataset.view));document.querySelectorAll('.tab').forEach(b=>b.onclick=()=>{document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));b.classList.add('active');state.resultTab=b.dataset.tab;renderResults()});
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.17 — Self Weight + Result Envelope ready');
})();
