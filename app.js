...
const state = {nodes:[],members:[],materials:JSON.parse(JSON.stringify(defaultMaterials)),sections:JSON.parse(JSON.stringify(defaultSections)),loadCases:JSON.parse(JSON.stringify(defaultLoadCases)),loadCombinations:JSON.parse(JSON.stringify(defaultLoadCombinations)),activeLoadCase:'DL',activeAnalysis:'CASE:DL', tool:'select', selected:null, memberStart:null, nextNode:1,nextMember:1, view:{scale:55,ox:120,oy:500}, dragging:null, panning:null, hover:null, results:null, resultTab:'summary', diagramScale:1, autoDiagramScale:true, showLabels:true,showLoadLabels:true,modelLoadLabels:true,resultsByAnalysis:new Map(),multiSelectedMemberIds:new Set(),boxSelect:null,building:{stories:0,bays:0,storyHeights:[],bayWidths:[],levels:[],grids:[]},layers:{members:true,nodes:true,loads:true,supports:true,labels:true},designSetup:{steelCode:'AISC 360',rcCode:'ACI 318',steelMethod:'LRFD',rcMethod:'Strength Design',designCombination:'ENVELOPE'}};
...
function projectSnapshot(){const model=cloneModel(),sum=memberLoadPersistenceSummaryV1182(model);return {version:'1.20-design-foundation',projectName:$('projectName')?.value||'Untitled Frame',units:$('units')?.value||'kN - m',...model,loadPersistence:{total:sum.total,manual:sum.manual,generated:sum.generated,fingerprint:sum.fingerprint}}}
