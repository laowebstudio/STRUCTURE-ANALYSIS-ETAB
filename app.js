...
const state = {nodes:[],members:[],materials:JSON.parse(JSON.stringify(defaultMaterials)),sections:JSON.parse(JSON.stringify(defaultSections)),loadCases:JSON.parse(JSON.stringify(defaultLoadCases)),loadCombinations:JSON.parse(JSON.stringify(defaultLoadCombinations)),activeLoadCase:'DL',activeAnalysis:'CASE:DL', tool:'select', selected:null, memberStart:null, nextNode:1,nextMember:1, view:{scale:55,ox:120,oy:500}, dragging:null, panning:null, hover:null, results:null, resultTab:'summary', diagramScale:1, autoDiagramScale:true, showLabels:true,resultsByAnalysis:new Map(),multiSelectedMemberIds:new Set(),boxSelect:null,building:{stories:0,bays:0,storyHeights:[],bayWidths:[],levels:[],grids:[]},layers:{members:true,nodes:true,loads:true,supports:true,labels:true}};
...
// V1.15 — load-independent global stiffness stability check.
...
// V1.15 — Structural Model Validation & Diagnostics
...
function buildingCenterV111(){
 ...
 wrap.innerHTML=`<div class="eng-card building-card"><div class="section-db-head"><div><h2>Building Center — V1.15.1 Fix</h2><small>2D Grid, Story Manager, Typical Floor, Floor/Wall Loads and Building Summary</small></div><button class="ml-close" id="v11Close">×</button></div>
...
function save(){const data={version:'1.15',projectName:$('projectName').value,units:$('units').value,...cloneModel()};const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=($('projectName').value||'sapudom-project').replace(/[^a-z0-9_-]+/gi,'-')+'.json';a.click();URL.revokeObjectURL(a.href);toast('ບັນທຶກໄຟລ໌ແລ້ວ')}
...
function modelingToolsV112(){const wrap=document.createElement('div');wrap.className='eng-dialog modeling-modal';wrap.innerHTML=`<div class="eng-card modeling-card"><div class="section-db-head"><div><h2>Drawing & Modeling Tools — V1.15.1 Fix</h2><small>Transform, copy and clean the selected structural model.</small></div><button class="ml-close" id="mtClose">×</button></div><div id="mtFeedback" class="modeling-feedback"><span class="feedback-dot"></span><div><b>Ready</b><small>Choose a selection or modeling command. Your last action will appear here.</small></div></div><div class="modeling-grid">
...
// V1.15 — Automatic Load Generator & Load Center
function memberUnitWeight(m){
 const sec=state.sections.find(x=>x.id===m.sectionId);
 const w=Number(m.weight ?? sec?.weight ?? 0);
...
function applySelfWeightV115({caseId,multiplier=1,scope='ALL',replace=true}){
 ...
 for(const m of members){const w=memberUnitWeight(m);const L=memberLength(m);if(!(w>0&&L>0)){skipped++;continue}m.loads=m.loads||{};m.loads[caseId]=m.loads[caseId]||[];const q=-Math.abs(w*multiplier);m.loads[caseId].push({type:'TRAP',w1:q,w2:q,a:0,b:L,direction:'GLOBAL_Y',source:'SELF_WEIGHT',multiplier,generatedBy:'V1.15'});applied++}
...
function loadCenterV115(){
 ...
 wrap.innerHTML=`<div class="eng-card load-center-card"><div class="section-db-head"><div><h2>⬇ Load Center — V1.15.1 Fix</h2><small>Automatic self weight, generated-load management and combination templates.</small></div><button class="ml-close" id="lc15Close">×</button></div>
...
$('releaseBtn').onclick=memberReleaseDialog;if($('loadCenterBtn'))$('loadCenterBtn').onclick=loadCenterV115;


// V1.15 — Collapsible Analysis Results and expanded Model Space
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.15.1 Fix — Professional menu ready');
})();


// V1.15.1 Fix — Professional menu and quick toolbar command routing
(function initProfessionalMenuV115Fix(){
  const closeMenus=()=>document.querySelectorAll('.menu-group.open').forEach(g=>g.classList.remove('open'));
  document.querySelectorAll('.menu-root').forEach(btn=>btn.addEventListener('click',e=>{e.stopPropagation();const g=btn.closest('.menu-group'),was=g.classList.contains('open');closeMenus();if(!was)g.classList.add('open')}));
  document.addEventListener('click',e=>{if(!e.target.closest('.menu-group'))closeMenus()});
  document.addEventListener('keydown',e=>{if(e.key==='Escape')closeMenus()});
  const trigger=id=>{const el=document.getElementById(id);if(el){el.click();closeMenus()}};
  document.querySelectorAll('[data-command]').forEach(b=>b.addEventListener('click',()=>trigger(b.dataset.command)));
  document.querySelectorAll('[data-tool-command]').forEach(b=>b.addEventListener('click',()=>{const t=document.querySelector(`.tool[data-tool="${b.dataset.toolCommand}"]`);if(t)t.click();closeMenus()}));
  document.querySelectorAll('[data-view-command]').forEach(b=>b.addEventListener('click',()=>{if(typeof setResultView==='function')setResultView(b.dataset.viewCommand);closeMenus()}));
})();
