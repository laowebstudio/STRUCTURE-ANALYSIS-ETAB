...
function frame3dCenterV127(){
 ...
 wrap.querySelector('#f3ApplyNode').onclick=()=>{const n=m3.nodes.find(n=>n.id==+wrap.querySelector('#f3NodeSelect').value);if(!n)return;n.restraints={};wrap.querySelectorAll('[data-rest]').forEach(q=>n.restraints[q.dataset.rest]=q.checked);for(const k of ['fx','fy','fz','mx','my','mz'])n.load[k]=+wrap.querySelector('#f3'+k).value||0;refresh();if(typeof integrated3dRefreshV128==='function')integrated3dRefreshV128(false)};
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.28.1 Fix — 3D Workspace sync fix • Phase 2 solver • 2D engine preserved');
})();
