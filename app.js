...
function cloneModel(){const model=JSON.parse(JSON.stringify({nodes:state.nodes,members:state.members,materials:state.materials,sections:state.sections,loadCases:state.loadCases,loadCombinations:state.loadCombinations,activeLoadCase:state.activeLoadCase,activeAnalysis:state.activeAnalysis,nextNode:state.nextNode,nextMember:state.nextMember,view:state.view,building:state.building,layers:state.layers}));return normalizeMemberLoadPersistenceV1182(model);}
function projectSnapshot(){const model=cloneModel(),sum=memberLoadPersistenceSummaryV1182(model);return {version:'1.18.2-fix',projectName:$('projectName')?.value||'Untitled Frame',units:$('units')?.value||'kN - m',...model,loadPersistence:{total:sum.total,manual:sum.manual,generated:sum.generated,fingerprint:sum.fingerprint}}}
...
function loadManagerV118(){
 ...
 wrap.innerHTML=`<div class="eng-card v118-card"><div class="section-db-head"><div><h2>☷ Load Assignment Manager — V1.18.2 Fix</h2><small>Review, filter, copy, assign and clear loads without editing Members one-by-one.</small></div><button class="ml-close" id="v118Close">×</button></div>
...
function loadCenterV115(){
 ...
 wrap.innerHTML=`<div class="eng-card load-center-card"><div class="section-db-head"><div><h2>⬇ Load Center — V1.18.2 Fix</h2><small>Automatic self weight, generated-load management and combination templates.</small></div><button class="ml-close" id="lc15Close">×</button></div>
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.18.2 Fix — Load Manager search/filter ready');
})();
