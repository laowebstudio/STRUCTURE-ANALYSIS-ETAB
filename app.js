...
// ===== V1.26.1 Fix Section Orientation / Rotate 90° =====
...
// V1.26.1 Fix.1 Fix — report maximum transverse displacement inside frame members,
...
function updateDiagramLegend(view){const el=$('diagramLegend');if(!state.results||view==='model'){el.hidden=true;return}el.hidden=false;if(view==='deformed'){const d=state.results.maxDispInfo,where=d?.memberId?`M${d.memberId} @ ${(d.ratio*100).toFixed(0)}%`:d?.nodeId?`Node ${d.nodeId}`:'—';el.innerHTML=`<b>Deformed Shape</b><div class="legend-row"><span>Max displacement</span><strong>${fmt(state.results.maxDisp*1000,3)} mm</strong></div><div class="legend-row"><span>Location</span><strong>${where}</strong></div><div class="legend-row"><span>Scale</span><strong>× ${effectiveDiagramScale('deformed').toFixed(1)}</strong></div><small>V1.26.1 Fix.1 • member-interior displacement included</small>`;return}const st=forceStats()[view],unit=view==='moment'?'kN·m':'kN',name={axial:'Axial Force (N)',shear:'Shear Force (V)',moment:'Bending Moment (M)'}[view];el.innerHTML=`<b>${name}</b><div class="legend-row legend-min"><span>Min</span><strong>${fmt(st.min,2)} ${unit}</strong></div><div class="legend-row legend-max"><span>Max</span><strong>${fmt(st.max,2)} ${unit}</strong></div><small>Scale × ${effectiveDiagramScale(view).toFixed(1)}${state.autoDiagramScale?' (Auto)':''} • Values ${state.showLabels?'ON':'OFF'}</small>`}
...
function designCenterV120(){
 const wrap=document.createElement('div');wrap.className='eng-dialog design-modal-v120';wrap.innerHTML=`<div class="eng-card design-card-v120"><div class="section-db-head"><div><h2>◆ Design Center — V1.26.1 Fix</h2><small>V1.26.1 Fix Section Orientation • Steel + RC Design preserved • 0° / 90° member orientation</small></div><button class="ml-close" id="d120Close">×</button></div><div id="d120Body"></div></div>`;document.body.appendChild(wrap);wrap.querySelector('#d120Close').onclick=()=>wrap.remove();wrap.onclick=e=>{if(e.target===wrap)wrap.remove()};const body=wrap.querySelector('#d120Body');
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.26.1 Fix — JSON/Cloud load visualization sync fixed • RC Beam demand fix preserved');
})();
