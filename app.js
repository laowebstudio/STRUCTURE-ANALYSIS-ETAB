...
function createWebGLPhysicalRendererV14712(canvas,m3,getFocus){
 ...
 function render(){
  ...
  const all=m3.nodes||[];let span=10;if(all.length){const xs=all.map(n=>n.x),ys=all.map(n=>n.y),zs=all.map(n=>n.z);span=Math.max(1,Math.max(...xs)-Math.min(...xs),Math.max(...ys)-Math.min(...ys),Math.max(...zs)-Math.min(...zs))}
  const target=[Number(m3.view.cx)||0,Number(m3.view.cy)||0,Number(m3.view.cz)||0],yaw=(Number(m3.view.yaw)||-35)*Math.PI/180,pitch=(Number(m3.view.pitch)||24)*Math.PI/180,zoom=Math.max(5,Number(m3.view.scale)||34),dist=Math.max(span*.75,span*2.35*(34/zoom));const eye=[target[0]+dist*Math.cos(pitch)*Math.cos(yaw),target[1]+dist*Math.cos(pitch)*Math.sin(yaw),target[2]+dist*Math.sin(pitch)];const P=perspective(42*Math.PI/180,W/H,Math.max(.03,dist/200),dist+span*12),V=lookAt(eye,target,[0,0,1]),MVP=mul4(P,V);
...
drawMemberLoadsV131();drawMemberLocalAxes();drawForceDiagrams();drawAxisTriad();drawLegend()}
 ...
 host.querySelector('#v1471Physical').onclick=()=>{physicalMode=true;m3.showLoads=false;host.querySelector('#v1471Physical').classList.add('primary');host.querySelector('#v1471Analysis').classList.remove('primary');host.querySelector('#v128TopStatus').textContent='V1.47.1.2 • WEBGL TRUE SOLID PHYSICAL MODEL • Beam + Column + Slab + Foundation';draw()};host.querySelector('#v1471Analysis').onclick=()=>{physicalMode=false;host.querySelector('#v1471Analysis').classList.add('primary');host.querySelector('#v1471Physical').classList.remove('primary');host.querySelector('#v128TopStatus').textContent='V1.47.1.2 • ANALYSIS MODEL • Centerlines + Loads + Results';draw()};
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.47.1.2 — WebGL True Solid Physical Model Engine');
})();
