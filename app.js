...
function updateDiagramLegend(view){
 ...
 const st=forceStats()[view],unit=view==='moment'?'kN·m':'kN',name={axial:'Axial Force (N)',shear:'Shear Force (V)',moment:'Bending Moment (M)'}[view];el.innerHTML=`<b>${name}</b><div class="legend-row legend-min"><span>Min</span><strong>${fmt(st.min,2)} ${unit}</strong></div><div class="legend-row legend-max"><span>Max</span><strong>${fmt(st.max,2)} ${unit}</strong></div><small>Scale × ${effectiveDiagramScale(view).toFixed(1)}${state.autoDiagramScale?' (Auto)':''} • Values ${state.showLabels?'ON':'OFF'}</small>`
}
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.26.2 Fix — Member-specific Max Displacement • Orientation + RC/Steel fixes preserved');
})();
