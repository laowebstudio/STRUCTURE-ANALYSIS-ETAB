...
function frame3dCenterV127(){
 ...
 const wrap=document.createElement('div');wrap.className='eng-dialog';wrap.innerHTML=`<div class="eng-card frame3d-card-v127"><div class="section-db-head"><div><h2>3D Frame Center — V1.27 Phase 1</h2><small>Independent 3D model • X/Y/Z coordinates • 6 DOF per node • JSON persistence • 2D engine protected</small></div><button class="ml-close">×</button></div>
...
$('frame3dBtn').onclick=frame3dCenterV127;

updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.27 — 3D Frame Phase 1 • verified 2D engine preserved');
})();
