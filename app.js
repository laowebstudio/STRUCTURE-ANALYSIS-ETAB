...
// V1.16 — load-independent global stiffness stability check.
...
// V1.16 — Structural Model Validation & Diagnostics
...
// V1.16 — reusable solver + Result Envelope / Critical Combination
...
function buildEnvelopeV116(specs){
 ...
 const rows=state.members.map(m=>{const vals=[];for(const r of results){const f=r.memberForces.find(x=>x.id===m.id);if(f)vals.push({r,f})}const ext=(getter)=>{let mn=null,mx=null;for(const x of vals)for(const v of getter(x.f)){if(!mn||v<mn.v)mn={v,spec:x.r.analysisSpec,label:x.r.analysisLabel};if(!mx||v>mx.v)mx={v,spec:x.r.analysisSpec,label:x.r.analysisLabel}}return{min:mn,max:mx}};return{id:m.id,N:ext(f=>[f.local[0],-f.local[3]]),V:ext(f=>[f.local[1],-f.local[4]]),M:ext(f=>[f.local[2],-f.local[5]])}});
...
function buildingCenterV111(){
 ...
 wrap.innerHTML=`<div class="eng-card building-card"><div class="section-db-head"><div><h2>Building Center — V1.16</h2><small>2D Grid, Story Manager, Typical Floor, Floor/Wall Loads and Building Summary</small></div><button class="ml-close" id="v11Close">×</button></div>
...
function modelingToolsV112(){const wrap=document.createElement('div');wrap.className='eng-dialog modeling-modal';wrap.innerHTML=`<div class="eng-card modeling-card"><div class="section-db-head"><div><h2>Drawing & Modeling Tools — V1.16</h2><small>Transform, copy and clean the selected structural model.</small></div><button class="ml-close" id="mtClose">×</button></div><div id="mtFeedback" class="modeling-feedback"><span class="feedback-dot"></span><div><b>Ready</b><small>Choose a selection or modeling command. Your last action will appear here.</small></div></div><div class="modeling-grid">
...
// V1.16 — Automatic Load Generator & Load Center
...
function applySelfWeightV115({caseId,multiplier=1,scope='ALL',replace=true}){
 ...
 for(const m of members){const w=memberUnitWeight(m);const L=memberLength(m);if(!(w>0&&L>0)){skipped++;continue}m.loads=m.loads||{};m.loads[caseId]=m.loads[caseId]||[];const q=-Math.abs(w*multiplier);m.loads[caseId].push({type:'TRAP',w1:q,w2:q,a:0,b:L,direction:'GLOBAL_Y',source:'SELF_WEIGHT',multiplier,generatedBy:'V1.16'});applied++}
...
function loadCenterV115(){
 ...
 wrap.innerHTML=`<div class="eng-card load-center-card"><div class="section-db-head"><div><h2>⬇ Load Center — V1.16</h2><small>Automatic self weight, generated-load management and combination templates.</small></div><button class="ml-close" id="lc15Close">×</button></div>
...
// V1.16 — Collapsible Analysis Results and expanded Model Space
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.16 — Result Envelope & Critical Combination ready');
})();
