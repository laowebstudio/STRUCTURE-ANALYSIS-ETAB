// SAPUDOM V1.46.3.1 — RC Beam Design Dialog Runtime Fix
window.SAPUDOM_BUILD_V14631='V1.46.3.1';
(() => {
'use strict';
...
function rcBeamRebar3DViewerV142(d){
  ...
  modal.innerHTML=`<div style="display:flex;align-items:center;justify-content:space-between;color:#fff;gap:12px;flex-wrap:wrap"><div><b style="font-size:20px">3D RC Rebar Viewer — M${d.id}</b><div style="font-size:12px;opacity:.78">V1.46.3.1 • RC Beam Design Dialog Runtime Fix</div></div><div style="display:flex;gap:7px;align-items:center;flex-wrap:wrap"><button id="v142fit">Fit</button><button id="v142iso">Isometric</button><button id="v142side">Side</button><button id="v142top">Top</button><button id="v142endi">End-i</button><button id="v142endj">End-j</button><label style="font-size:12px"><input id="v142conc" type="checkbox" checked> Concrete</label><label style="font-size:12px"><input id="v142st" type="checkbox" checked> Stirrups</label><label style="font-size:12px"><input id="v142bars" type="checkbox" checked> Bottom bars</label><label style="font-size:12px"><input id="v143topbars" type="checkbox" checked> Top bars</label><button id="v142close">Close</button></div></div><div style="position:relative;flex:1;min-height:360px;background:#e8eef5;border-radius:8px;overflow:hidden"><canvas id="v142canvas" style="width:100%;height:100%;display:block;touch-action:none"></canvas><div id="v142info" style="position:absolute;left:12px;top:12px;background:rgba(255,255,255,.94);padding:10px 12px;border-radius:8px;box-shadow:0 2px 8px #0002;font:12px Arial;line-height:1.55;color:#0f172a;max-width:360px"></div><div style="position:absolute;right:12px;bottom:10px;background:rgba(15,23,42,.86);color:white;padding:7px 10px;border-radius:7px;font:11px Arial">Drag: Rotate • Wheel: Zoom • V1.46.3.1 station-envelope RC zoning viewer</div></div>`;
  document.body.appendChild(modal);
...
function rcBeamDesignErrorDialogV14631(err){
  console.error('V1.46.3.1 RC Beam Design runtime error',err);
  ...
  w.innerHTML=`<div style="width:min(680px,94vw);background:#fff;border-radius:16px;box-shadow:0 24px 70px #0006;overflow:hidden"><div style="padding:16px 18px;background:#991b1b;color:#fff;display:flex;justify-content:space-between;gap:12px"><div><b style="font-size:19px">RC Beam Design could not open</b><div style="font-size:12px;opacity:.9">SAPUDOM V1.46.3.1 runtime guard</div></div><button id="v14631RcErrX" style="width:38px;height:38px">×</button></div><div style="padding:16px 18px;font-size:13px;line-height:1.55"><b>Runtime message</b><div style="margin-top:6px;padding:10px;background:#fef2f2;border:1px solid #fecaca;border-radius:8px;font-family:monospace;word-break:break-word">${msg}</div><div style="margin-top:12px;color:#475569">This dialog confirms the button event is working. V1.46.3.1 no longer fails silently if RC demand generation throws an exception.</div></div><div style="padding:12px 18px;border-top:1px solid #e2e8f0;text-align:right"><button id="v14631RcErrClose">Close</button></div></div>`;
...
function rcBeamDesignCenterV141(){
  ...
  w.innerHTML=`<div style="width:min(1120px,96vw);max-height:93vh;background:#fff;border-radius:18px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 24px 70px #0005">
    <header style="padding:18px 20px;background:#173b68;color:#fff;display:flex;justify-content:space-between"><div>
      ...
      <div style="font-size:13px;opacity:.84">V1.46.3.1 • RC Beam Design Dialog Runtime Fix</div></div>
  ...
  w.querySelector('#v141Apply').onclick=()=>{
    ...
    designs=rcBeamDesignV141();w.querySelector('#v141tbody').innerHTML=rows();bindDetails();toast('V1.46.3.1 recalculated • Whole Model → Strength Combos → Station Envelope → RC Design → Rebar Viewer');
  };
}
