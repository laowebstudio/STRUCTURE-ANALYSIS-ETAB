  ...
  <div class="v1364ComboModal" style="width:min(960px,96vw);max-height:90vh;background:#f8fafc;border-radius:18px;box-shadow:0 24px 70px rgba(15,23,42,.28);display:flex;flex-direction:column;overflow:hidden;border:1px solid rgba(255,255,255,.7)">
    <header style="flex:0 0 auto;display:flex;justify-content:space-between;align-items:flex-start;gap:16px;padding:18px 20px;background:linear-gradient(135deg,#0f2747,#173b68);color:#fff">
      <div>
        ...
        <div style="margin-top:4px;font-size:13px;opacity:.82">V1.41.4 • RC Beam Detailing Drawing</div>
      </div>
...
// ===== V1.41.4 — RC Beam Detailing Drawing =====
function rcBeamDetailingDrawingV1414(d){
  const m3=ensure3DLoadSystemV131();
  const ni=(m3.nodes||[]).find(n=>String(n.id)===String(d.i));
  const nj=(m3.nodes||[]).find(n=>String(n.id)===String(d.j));
  const Lm=(ni&&nj)?Math.hypot((+nj.x||0)-(+ni.x||0),(+nj.y||0)-(+ni.y||0),(+nj.z||0)-(+ni.z||0)):4;
  const L=Math.max(500,Math.round(Lm*1000));
  const nBars=d.nBars||0, dia=d.cfg.mainBarDia, stDia=d.cfg.stirrupDia, stSp=Math.round(d.sReq);
  const layers=Number.isFinite(d.detailing.layers)?d.detailing.layers:0;
  const barText=nBars?`${nBars}Ø${dia}`:'REVIEW SECTION';
  const status=d.overall.pass?'PASS':'REVIEW / NOT VERIFIED';
  const esc=x=>String(x??'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));
  const W=1400,H=900, x0=135,x1=1050,yBeam=255,beamH=110;
  const sx=(x1-x0)/L;
  const dim=(x,y,txt)=>`<line x1="${x0}" y1="${y}" x2="${x1}" y2="${y}" class="dim"/><line x1="${x0}" y1="${y-10}" x2="${x0}" y2="${y+10}" class="thin"/><line x1="${x1}" y1="${y-10}" x2="${x1}" y2="${y+10}" class="thin"/><text x="${(x0+x1)/2}" y="${y-7}" class="t mid">${txt}</text>`;
  const stirrups=[]; for(let mm=0;mm<=L;mm+=Math.max(75,stSp)){const x=x0+mm*sx;stirrups.push(`<line x1="${x}" y1="${yBeam}" x2="${x}" y2="${yBeam+beamH}" class="st"/>`)}
  const bars=[];
  if(nBars){
    const per=Math.max(1,d.detailing.barsPerLayer||nBars), rows=Math.max(1,layers||Math.ceil(nBars/per)); let left=nBars;
    for(let r=0;r<rows;r++){const cnt=Math.min(per,left);left-=cnt; const yy=565-r*30; for(let k=0;k<cnt;k++){const xx=235+(cnt===1?120:k*(240/Math.max(1,cnt-1)));bars.push(`<circle cx="${xx}" cy="${yy}" r="7" class="bar"/>`)}}
  }
  const svg=`<svg viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
    <style>.sheet{fill:white}.line{stroke:#111;stroke-width:2;fill:none}.thin{stroke:#111;stroke-width:1}.dim{stroke:#111;stroke-width:1;marker-start:url(#a);marker-end:url(#a)}.rebar{stroke:#15803d;stroke-width:4}.st{stroke:#16a34a;stroke-width:1}.bar{fill:#15803d;stroke:#064e3b;stroke-width:1}.t{font:16px Arial,sans-serif;fill:#111}.sm{font:13px Arial,sans-serif}.xs{font:11px Arial,sans-serif}.b{font-weight:700}.green{fill:#15803d}.red{fill:#b91c1c}.mid{text-anchor:middle}.title{font:700 22px Arial,sans-serif}</style>
    <defs><marker id="a" markerWidth="8" markerHeight="8" refX="4" refY="4" orient="auto"><path d="M8,0 L0,4 L8,8" fill="none" stroke="#111"/></marker></defs>
    <rect width="1400" height="900" class="sheet"/><rect x="25" y="25" width="1350" height="850" class="line"/>
    <text x="55" y="65" class="title">RC BEAM DETAILING DRAWING — M${esc(d.id)}</text><text x="55" y="90" class="t">N${esc(d.i)} → N${esc(d.j)} • V1.41.4</text>
    <text x="${(x0+x1)/2}" y="145" class="t b mid">BEAM ELEVATION — M${esc(d.id)} (${d.cfg.b}×${d.cfg.h} mm)</text>
    <rect x="${x0}" y="${yBeam}" width="${x1-x0}" height="${beamH}" class="line"/>${stirrups.join('')}
    <line x1="${x0+10}" y1="${yBeam+20}" x2="${x1-10}" y2="${yBeam+20}" class="rebar"/><line x1="${x0+10}" y1="${yBeam+beamH-20}" x2="${x1-10}" y2="${yBeam+beamH-20}" class="rebar"/>
    <text x="${(x0+x1)/2}" y="${yBeam+18}" class="sm green mid">Top construction bars / verify by engineer</text><text x="${(x0+x1)/2}" y="${yBeam+beamH-26}" class="t b green mid">${esc(barText)} MAIN TENSION STEEL</text>
    <text x="${(x0+x1)/2}" y="${yBeam+beamH+30}" class="t b green mid">Ø${stDia} @ ${stSp} mm STIRRUPS</text>${dim(x0,420,`L = ${L} mm`)}
    <text x="300" y="485" class="t b mid">SECTION A-A</text><rect x="210" y="510" width="180" height="${Math.min(240,Math.max(150,d.cfg.h*.34))}" class="line"/><rect x="225" y="525" width="150" height="${Math.min(210,Math.max(120,d.cfg.h*.34-30))}" class="st" fill="none"/>${bars.join('')}
    <text x="300" y="${Math.min(240,Math.max(150,d.cfg.h*.34))+535}" class="t mid">b = ${d.cfg.b} mm • h = ${d.cfg.h} mm</text><text x="300" y="${Math.min(240,Math.max(150,d.cfg.h*.34))+558}" class="sm green mid">${esc(barText)} • Ø${stDia}@${stSp}</text>
    <text x="520" y="500" class="t b">DESIGN / DETAILING DATA</text><text x="520" y="530" class="t">Mu = ${d.Mu.toFixed(1)} kN·m   •   φMn = ${Number.isFinite(d.flexure.phiMn)?d.flexure.phiMn.toFixed(1):'—'} kN·m</text><text x="520" y="557" class="t">Vu = ${d.Vu.toFixed(1)} kN   •   φVn = ${d.shear.phiVn.toFixed(1)} kN</text><text x="520" y="584" class="t">As req = ${Number.isFinite(d.AsReq)?d.AsReq.toFixed(0):'REVIEW'} mm²   •   As prov = ${Number.isFinite(d.AsProv)?d.AsProv.toFixed(0):'—'} mm²</text><text x="520" y="611" class="t">Cover = ${d.cfg.cover} mm   •   fc' = ${d.cfg.fc} MPa   •   fy = ${d.cfg.fy} MPa</text><text x="520" y="638" class="t">Layers = ${layers||'REVIEW'}   •   Clear spacing ≈ ${Number.isFinite(d.detailing.actualClear)?d.detailing.actualClear.toFixed(1):'—'} mm</text>
    <rect x="1090" y="25" width="285" height="850" class="line"/><line x1="1090" y1="125" x2="1375" y2="125" class="thin"/><line x1="1090" y1="250" x2="1375" y2="250" class="thin"/><line x1="1090" y1="390" x2="1375" y2="390" class="thin"/><line x1="1090" y1="555" x2="1375" y2="555" class="thin"/><line x1="1090" y1="720" x2="1375" y2="720" class="thin"/>
    <text x="1110" y="60" class="t b">SAPUDOM</text><text x="1110" y="85" class="sm">Structure Analysis</text><text x="1110" y="110" class="sm">RC Beam Detailing</text><text x="1110" y="155" class="sm b">Member</text><text x="1110" y="180" class="t">M${esc(d.id)} • N${esc(d.i)}→N${esc(d.j)}</text><text x="1110" y="215" class="sm">Section ${d.cfg.b}×${d.cfg.h} mm</text>
    <text x="1110" y="280" class="sm b">Governing Moment</text><text x="1110" y="305" class="sm">${esc(d.govM.axis)}-${esc(d.govM.end)} = ${d.Mu.toFixed(1)} kN·m</text><text x="1110" y="330" class="xs">${esc(d.govM.combo)}</text><text x="1110" y="360" class="sm b">Governing Shear</text><text x="1110" y="382" class="sm">${esc(d.govV.axis)}-${esc(d.govV.end)} = ${d.Vu.toFixed(1)} kN</text>
    <text x="1110" y="425" class="sm b">Reinforcement</text><text x="1110" y="455" class="t green">${esc(barText)}</text><text x="1110" y="485" class="t green">Ø${stDia} @ ${stSp} mm</text><text x="1110" y="520" class="sm">Cover ${d.cfg.cover} mm</text>
    <text x="1110" y="590" class="sm b">Verification</text><text x="1110" y="620" class="t ${d.flexure.status==='PASS'?'green':'red'}">Flexure: ${esc(d.flexure.status)}</text><text x="1110" y="650" class="t ${d.shear.status==='PASS'?'green':'red'}">Shear: ${esc(d.shear.status)}</text><text x="1110" y="680" class="t ${d.detailing.pass?'green':'red'}">Detailing: ${esc(d.detailing.status)}</text>
    <text x="1110" y="755" class="sm b">Overall</text><text x="1110" y="790" class="t b ${d.overall.pass?'green':'red'}">${esc(status)}</text><text x="1110" y="830" class="xs">Scale: schematic / NTS</text><text x="1110" y="850" class="xs">V1.41.4 RC Beam Detailing Drawing</text>
  </svg>`;
  const o=document.createElement('div');o.style.cssText='position:fixed;inset:0;z-index:100004;background:#0f172aeF;padding:14px;display:flex;flex-direction:column;gap:10px';
  o.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;color:white"><div><b style="font-size:20px">RC Beam Detailing Drawing — M${d.id}</b><div style="font-size:12px;opacity:.8">V1.41.4 • Drawing generated from current RC Beam Design result</div></div><div style="display:flex;gap:8px"><button id="v1414print">Print / Save PDF</button><button id="v1414svg">Download SVG</button><button id="v1414close">Close</button></div></div><div id="v1414sheet" style="background:#fff;flex:1;overflow:auto;border-radius:8px">${svg}</div>`;
  document.body.appendChild(o); o.querySelector('#v1414close').onclick=()=>o.remove();
  o.querySelector('#v1414svg').onclick=()=>{const blob=new Blob([svg],{type:'image/svg+xml'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`SAPUDOM-M${d.id}-RC-Beam-Detailing-V1.41.4.svg`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),500)};
  o.querySelector('#v1414print').onclick=()=>{const pw=window.open('','_blank');if(!pw){toast('Allow pop-ups to print drawing');return}pw.document.write(`<html><head><title>RC Beam M${d.id} Detailing</title><style>@page{size:A3 landscape;margin:6mm}body{margin:0}svg{width:100%;height:auto}</style></head><body>${svg}</body></html>`);pw.document.close();pw.focus();setTimeout(()=>pw.print(),250)};
}
...
function rcBeamDesignCenterV141(){
  ...
  w.innerHTML=`<div style="width:min(1120px,96vw);max-height:93vh;background:#fff;border-radius:18px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 24px 70px #0005">
    <header style="padding:18px 20px;background:#173b68;color:#fff;display:flex;justify-content:space-between"><div>
      ...
      <div style="font-size:13px;opacity:.84">V1.41.4 • RC Beam Detailing Drawing</div></div>
