...
const state = {nodes:[],members:[],materials:JSON.parse(JSON.stringify(defaultMaterials)),sections:JSON.parse(JSON.stringify(defaultSections)),loadCases:JSON.parse(JSON.stringify(defaultLoadCases)),loadCombinations:JSON.parse(JSON.stringify(defaultLoadCombinations)),activeLoadCase:'DL',activeAnalysis:'CASE:DL', tool:'select', selected:null, memberStart:null, nextNode:1,nextMember:1, view:{scale:55,ox:120,oy:500}, dragging:null, panning:null, hover:null, results:null, resultTab:'summary', diagramScale:1, autoDiagramScale:true, showLabels:true,showLoadLabels:true,modelLoadLabels:true,resultsByAnalysis:new Map(),multiSelectedMemberIds:new Set(),boxSelect:null,building:{stories:0,bays:0,storyHeights:[],bayWidths:[],levels:[],grids:[]},layers:{members:true,nodes:true,loads:true,supports:true,labels:true},designSetup:{steelCode:'AISC 360-22',rcCode:'ACI 318',steelMethod:'LRFD',rcMethod:'Strength Design',designCombination:'ENVELOPE'}};
...
function restore(s){s=normalizeMemberLoadPersistenceV1182(JSON.parse(JSON.stringify(s||{})));state.nodes=s.nodes||[];state.members=s.members||[];state.materials=s.materials||JSON.parse(JSON.stringify(defaultMaterials));state.sections=s.sections||JSON.parse(JSON.stringify(defaultSections));mergePersistentLibraries();state.loadCases=s.loadCases||JSON.parse(JSON.stringify(defaultLoadCases));state.loadCombinations=s.loadCombinations||JSON.parse(JSON.stringify(defaultLoadCombinations));state.activeLoadCase=s.activeLoadCase||state.loadCases[0]?.id||'DL';state.activeAnalysis=s.activeAnalysis||('CASE:'+state.activeLoadCase);migrateLoads();migrateMemberReleases();state.nextNode=s.nextNode||1;state.nextMember=s.nextMember||1;state.view=s.view||state.view;state.building=s.building||{stories:0,bays:0,storyHeights:[],bayWidths:[],levels:[],grids:[]};state.layers=s.layers||{members:true,nodes:true,loads:true,supports:true,labels:true};state.designSetup=s.designSetup||{steelCode:'AISC 360-22',rcCode:'ACI 318',steelMethod:'LRFD',rcMethod:'Strength Design',designCombination:'ENVELOPE'};state.selected=null;state.multiSelectedMemberIds=new Set();state.boxSelect=null;state.memberStart=null;state.results=null;state.resultsByAnalysis=new Map();setResultView('model',false);updateResultModeButtons();render();updateUI();renderResults();refreshLayoutAfterLoad();}
...
// ===== V1.23 Steel Design Phase 3 — AISC 360-22 LRFD =====
...
function steelDesignV122(m){
 ...
 const rx=Number(sec.rx||(A>0&&I>0?Math.sqrt(I/A):0)),ry=Number(sec.ry||(A>0&&Iy>0?Math.sqrt(Iy/A):0));
...
function steelDetailV122(m){
 ...
 const wrap=document.createElement('div');wrap.className='eng-dialog';wrap.innerHTML=`<div class="eng-card steel-detail-v122"><div class="section-db-head"><div><h2>Steel Design Detail — M${m.id}</h2><small>${sec.name||m.sectionId||'Section'} • ${d.memberType} • AISC 360-22 LRFD Phase 3</small></div><button class="ml-close">×</button></div><div class="steel-detail-grid">
...
function designDemandV120(m){
 ...
 for(const [name,r] of pool){const f=r?.memberForces?.find(x=>x.id===m.id);if(!f)continue;const N=Math.max(Math.abs(f.local?.[0]||0),Math.abs(f.local?.[3]||0)),V=Math.max(Math.abs(f.local?.[1]||0),Math.abs(f.local?.[4]||0)),M=Math.max(Math.abs(f.local?.[2]||0),Math.abs(f.local?.[5]||0));if(M>best.M||V>best.V||N>best.N)best={N,V,M,analysis:name}}
 return best;
}
function designCenterV120(){
 const wrap=document.createElement('div');wrap.className='eng-dialog design-modal-v120';wrap.innerHTML=`<div class="eng-card design-card-v120"><div class="section-db-head"><div><h2>◆ Design Center — V1.23</h2><small>Steel Design Phase 3 • AISC 360-22 LRFD • traceable strong-axis flexure/LTB + compactness + interaction</small></div><button class="ml-close" id="d120Close">×</button></div><div id="d120Body"></div></div>`;document.body.appendChild(wrap);wrap.querySelector('#d120Close').onclick=()=>wrap.remove();wrap.onclick=e=>{if(e.target===wrap)wrap.remove()};const body=wrap.querySelector('#d120Body');
 ...
 <section class="design-results-v120"><div class="section-db-head"><div><h3>Steel Design Results — Phase 3</h3><small>Steel I strong-axis flexure with explicit Yielding / Inelastic LTB / Elastic LTB / Flange Local Buckling paths. Click a Member ID, then Show Calculation for the trace. Unsupported cases show WARNING.</small></div><div class="design-actions-v121"><button id="dRunSteel" class="primary">▶ Run Steel Design</button><button id="dCsv">Export Steel Design CSV</button></div></div><div class="design-table-wrap"><table><thead><tr><th>Member</th><th>Type</th><th>Material</th><th>Pu kN</th><th>φPn kN</th><th>Axial D/C</th><th>Mu kN·m</th><th>φMn kN·m</th><th>Flex D/C</th><th>Interaction</th><th>Governing</th><th>Combination</th><th>Status</th></tr></thead><tbody>${rows||'<tr><td colspan="13">No members</td></tr>'}</tbody></table></div></section>`;
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.23 — Steel Design Phase 3 ready');
})();
