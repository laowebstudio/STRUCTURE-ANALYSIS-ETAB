...
function steelDetailV122(m){
 ...
 const wrap=document.createElement('div');wrap.className='eng-dialog';wrap.innerHTML=`<div class="eng-card steel-detail-v122"><div class="section-db-head"><div><h2>Steel / Beam-Column Design Detail — M${m.id}</h2><small>${sec.name||m.sectionId||'Section'} • ${d.memberType} • AISC 360-22 LRFD V1.24</small></div><button class="ml-close">×</button></div><div class="steel-detail-grid">
...
function designCenterV120(){
 const wrap=document.createElement('div');wrap.className='eng-dialog design-modal-v120';wrap.innerHTML=`<div class="eng-card design-card-v120"><div class="section-db-head"><div><h2>◆ Design Center — V1.24</h2><small>Steel Column + Beam-Column • AISC 360-22 LRFD • axis-aware compression + strong-axis flexure/LTB + H1 interaction</small></div><button class="ml-close" id="d120Close">×</button></div><div id="d120Body"></div></div>`;document.body.appendChild(wrap);wrap.querySelector('#d120Close').onclick=()=>wrap.remove();wrap.onclick=e=>{if(e.target===wrap)wrap.remove()};const body=wrap.querySelector('#d120Body');
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.24 — Steel Column + Beam-Column Design ready');
})();
