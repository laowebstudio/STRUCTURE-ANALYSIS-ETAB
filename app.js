...
// ===== V1.33 — Story Response & Drift (post-processing only; V1.32 solver protected) =====
function storyResponseV133(m3,res){
  ...
  for(let st=1;st<=stories;st++){
    ...
    let ux=0,uy=0,driftX=0,driftY=0,govNode=null;
    for(const n of cur){
      const d=byId.get(Number(n.id)); if(!d)continue;
      if(Math.abs(d.ux)>=Math.abs(ux))ux=d.ux;
      if(Math.abs(d.uy)>=Math.abs(uy))uy=d.uy;
      const pn=prevGrid.get(`${n.gridX??''}:${n.gridY??''}`),pd=pn&&byId.get(Number(pn.id));
      const dx=d.ux-(pd?.ux||0),dy=d.uy-(pd?.uy||0);
      if(Math.abs(dx)>=Math.abs(driftX)){driftX=dx;govNode=n.id}
      if(Math.abs(dy)>=Math.abs(driftY)){driftY=dy;if(Math.abs(dy)>Math.abs(driftX))govNode=n.id}
    }
    ...
    rows.push({story:st,elevation:z,height:h,ux,uy,driftX,driftY,ratioX:rx,ratioY:ry,governingRatio:Math.max(rx,ry),direction:rx>=ry?'X':'Y',nodeId:govNode});
  }
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.33 — Story Response & Drift • V1.32 solver protected');
})();
