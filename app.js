...
function initIntegrated3DV128(host){
 ...
 function draw(){const r=c.getBoundingClientRect(),d=devicePixelRatio||1;c.width=Math.max(1,r.width*d);c.height=Math.max(1,r.height*d);cx.setTransform(d,0,0,d,0,0);cx.clearRect(0,0,r.width,r.height);cx.fillStyle='#f8fafc';cx.fillRect(0,0,r.width,r.height);const grd=cx.createRadialGradient(r.width*.5,r.height*.42,20,r.width*.5,r.height*.45,Math.max(r.width,r.height)*.65);grd.addColorStop(0,'rgba(255,255,255,.85)');grd.addColorStop(1,'rgba(226,232,240,.35)');cx.fillStyle=grd;cx.fillRect(0,0,r.width,r.height);cx.strokeStyle='rgba(148,163,184,.20)';for(let k=-12;k<=12;k++){cx.beginPath();cx.moveTo(0,r.height/2+k*25);cx.lineTo(r.width,r.height/2+k*25);cx.stroke();cx.beginPath();cx.moveTo(r.width/2+k*25,0);cx.lineTo(r.width/2+k*25,r.height);cx.stroke()}
...
drawMemberLoadsV131();drawMemberLocalAxes();drawForceDiagrams();drawAxisTriad();drawLegend()}
 ...
 host.querySelector('#v1471Physical').onclick=()=>{physicalMode=true;m3.showLoads=false;host.querySelector('#v1471Physical').classList.add('primary');host.querySelector('#v1471Analysis').classList.remove('primary');host.querySelector('#v128TopStatus').textContent='V1.47.1.1 • REALISTIC PHYSICAL MODEL • Beam + Column + Slab + Foundation';draw()};host.querySelector('#v1471Analysis').onclick=()=>{physicalMode=false;host.querySelector('#v1471Analysis').classList.add('primary');host.querySelector('#v1471Physical').classList.remove('primary');host.querySelector('#v128TopStatus').textContent='V1.47.1.1 • ANALYSIS MODEL • Centerlines + Loads + Results';draw()};
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.47.1.1 — Realistic Physical Building Geometry & Rendering');
})();
