...
function frame3dCenterV127(){
 ...
 function physicalDimsV1471(mm){const A=Math.max(1e-9,Number(mm.A)||.15),Iy=Math.max(1e-12,Number(mm.Iy)||.001),Iz=Math.max(1e-12,Number(mm.Iz)||.001);let h=Math.sqrt(12*Iy/A),b=Math.sqrt(12*Iz/A);if(String(mm.memberType||'').startsWith('Column')){const d=Math.sqrt(A);b=d;h=d}return {b:Math.max(.12,Math.min(1.5,b)),h:Math.max(.12,Math.min(1.5,h))}}
 function physicalMemberV1471(mm,a,bn,r,def){
  const dim=physicalDimsV1471(mm),isCol=String(mm.memberType||'').startsWith('Column');
  const dx=bn.x-a.x,dy=bn.y-a.y,dz=bn.z-a.z,Lxy=Math.hypot(dx,dy),L=Math.hypot(dx,dy,dz)||1;
  let ux,uy,uz,vx,vy,vz;
  if(isCol||Lxy<1e-7){ux=1;uy=0;uz=0;vx=0;vy=1;vz=0}else{ux=-dy/Lxy;uy=dx/Lxy;uz=0;vx=0;vy=0;vz=1}
  const hb=dim.b/2,hh=dim.h/2;
  const corner=(n,su,sv)=>({x:n.x+ux*hb*su+vx*hh*sv,y:n.y+uy*hb*su+vy*hh*sv,z:n.z+uz*hb*su+vz*hh*sv});
  const P=[corner(a,-1,-1),corner(a,1,-1),corner(a,1,1),corner(a,-1,1),corner(bn,-1,-1),corner(bn,1,-1),corner(bn,1,1),corner(bn,-1,1)].map(q=>proj(q,r.width,r.height,def));
  const faces=[[0,1,2,3],[4,7,6,5],[0,4,5,1],[1,5,6,2],[2,6,7,3],[3,7,4,0]];
  const selected=focusTarget?.type==='member'&&focusTarget.id===mm.id;
  const shades=selected?['rgba(220,38,38,.90)','rgba(185,28,28,.88)','rgba(239,68,68,.86)']:isCol?['rgba(176,170,158,.98)','rgba(151,145,135,.98)','rgba(199,193,181,.98)']:['rgba(187,181,169,.98)','rgba(159,153,143,.98)','rgba(210,204,191,.98)'];
  const sorted=faces.map((f,i)=>({f,i,y:f.reduce((z,k)=>z+P[k].y,0)/4})).sort((a,b)=>a.y-b.y);
  cx.save();cx.lineJoin='round';cx.lineWidth=.8;cx.strokeStyle=selected?'#991b1b':'rgba(82,78,70,.72)';
  sorted.forEach((o,j)=>{cx.fillStyle=shades[(o.i+j)%shades.length];cx.beginPath();o.f.forEach((k,i)=>i?cx.lineTo(P[k].x,P[k].y):cx.moveTo(P[k].x,P[k].y));cx.closePath();cx.fill();cx.stroke()});cx.restore()
 }
...
function building3dCenterV130(){
  ...
  wrap.innerHTML=`<div class="eng-card v130-building-card"><div class="section-db-head"><div><h2>Full Building Generator — V1.47.2.1</h2><small>Physical Beam + Column + Slab + Foundation model • whole-building frame solve • 2D model protected</small></div><button class="ml-close">×</button></div>
  <div class="v130-building-grid">
   ...
   <section><h3>Stories</h3>
    ...
    <label class="v130-check"><input id="v147Slabs" type="checkbox" ${b.generateSlabs!==false?'checked':''}> Generate + analyze floor slabs</label>
    <div class="engineering-note"><b>Easy Slab Setup:</b> Enter slab thickness and gravity loads as positive values. SAPUDOM applies gravity automatically in Global −Z, includes self-weight when checked, and validates the inputs before creating the shell/plate model.</div>
    <div class="v130-mini"><label>Slab thickness (mm)<input id="v147SlabT" type="number" min="80" max="500" step="10" value="${Number(b.slabThicknessMm)||150}"></label><label>Concrete E (MPa)<input id="v147SlabE" type="number" min="10000" max="60000" step="500" value="${Number(b.slabE_MPa)||25000}"></label></div>
    <div class="v130-mini"><label>Superimposed DL ↓ (kN/m²)<input id="v147SlabDL" type="number" min="0" step="0.1" value="${Math.abs(Number(b.slabDL)||4.5)}"></label><label>LL ↓ (kN/m²)<input id="v147SlabLL" type="number" min="0" step="0.1" value="${Math.abs(Number(b.slabLL)||2.0)}"></label></div>
    <div class="v130-mini"><label>Poisson ν<input id="v147SlabNu" type="number" min="0" max="0.49" step="0.01" value="${Number.isFinite(Number(b.slabNu))?Number(b.slabNu):0.20}"></label><label>Concrete unit weight (kN/m³)<input id="v147SlabGamma" type="number" min="10" max="30" step="0.5" value="${Number(b.slabGamma)||24}"></label></div>
    <label class="v130-check"><input id="v147SlabSW" type="checkbox" ${b.slabSelfWeight!==false?'checked':''}> Include slab self-weight automatically in DL</label>
    <div id="v147SlabLoadPreview" class="engineering-note"></div>
  ...
  <div class="engineering-note"><b>V1.47.2.1:</b> Gravity inputs are positive magnitudes; the solver applies them in Global −Z. Story-height count, slab material properties, load signs, and slab thickness are validated before generation.</div></div>`;
  document.body.appendChild(wrap);
  ...
  const syncStoryHeights=()=>{const st=Math.max(1,Math.round(Number(q('v130Stories').value)||1));let a=String(q('v130Hs').value||'').split(',').map(x=>Number(x.trim())).filter(x=>Number.isFinite(x)&&x>0);if(!a.length)a=[3.5];if(a.length<st)a=a.concat(Array(st-a.length).fill(a.at(-1)||3.5));if(a.length>st)a=a.slice(0,st);q('v130Hs').value=a.join(',')};
  const slabPreview=()=>{const t=Number(q('v147SlabT')?.value),gamma=Number(q('v147SlabGamma')?.value),dl=Number(q('v147SlabDL')?.value),ll=Number(q('v147SlabLL')?.value),sw=!!q('v147SlabSW')?.checked,box=q('v147SlabLoadPreview');if(!box)return;const self=(Number.isFinite(t)&&Number.isFinite(gamma)?t/1000*gamma:0),total=(Math.max(0,Number.isFinite(dl)?dl:0)+(sw?self:0));box.innerHTML=`<b>Gravity load preview</b><br>Self-weight = ${self.toFixed(2)} kN/m²${sw?' (included in DL)':' (not included)'} • Total DL = ${total.toFixed(2)} kN/m² ↓ • LL = ${Math.max(0,Number.isFinite(ll)?ll:0).toFixed(2)} kN/m² ↓<br><small>You enter positive magnitudes only; SAPUDOM applies Global −Z automatically.</small>`};
  const validateSlabInputs=()=>{if(!q('v147Slabs')?.checked)return;const t=Number(q('v147SlabT').value),E=Number(q('v147SlabE').value),nu=Number(q('v147SlabNu').value),gamma=Number(q('v147SlabGamma').value),dl=Number(q('v147SlabDL').value),ll=Number(q('v147SlabLL').value);if(!(t>=80&&t<=500))throw new Error('Slab thickness must be 80–500 mm.');if(!(E>=10000&&E<=60000))throw new Error('Concrete E must be 10,000–60,000 MPa.');if(!(nu>=0&&nu<0.5))throw new Error('Poisson ν must satisfy 0 ≤ ν < 0.5.');if(!(gamma>0&&gamma<=30))throw new Error('Concrete unit weight must be > 0 and ≤ 30 kN/m³.');if(!(dl>=0&&ll>=0))throw new Error('DL and LL must be entered as positive downward magnitudes. Do not type a minus sign.')};
  const preview=()=>{try{const bx=+q('v130Bx').value,by=+q('v130By').value,st=+q('v130Stories').value;parseSeries(q('v130Wx').value,bx,'X widths');parseSeries(q('v130Wy').value,by,'Y widths');parseSeries(q('v130Hs').value,st,'Story heights');validateSlabInputs();const nodes=(bx+1)*(by+1)*(st+1);const cols=(bx+1)*(by+1)*st;const beams=st*((bx*(by+1))+(by*(bx+1)));const slabs=q('v147Slabs')?.checked?st*bx*by:0,foots=q('v147Footings')?.checked?(bx+1)*(by+1):0;q('v130Preview').innerHTML=`<b>${st}-Story / ${bx}×${by} Bay Full Building</b><span>Nodes ${nodes} • Columns ${cols} • Beams ${beams} • Slabs ${slabs} • Footings ${foots} • Frame DOF ${nodes*6}</span>`}catch(e){q('v130Preview').innerHTML=`<b>Check inputs</b><span>${e.message}</span>`}slabPreview()};
  q('v130Stories')?.addEventListener('change',()=>{syncStoryHeights();preview()});
  ['v130Bx','v130By','v130Wx','v130Wy','v130Hs','v147Slabs','v147Footings','v147SlabT','v147SlabE','v147SlabDL','v147SlabLL','v147SlabNu','v147SlabGamma','v147SlabSW'].forEach(id=>q(id)?.addEventListener('input',preview)); syncStoryHeights(); preview();
  ...
  q('v130Generate').onclick=()=>{try{
    ...
    syncStoryHeights(); validateSlabInputs();
    ...
    m3.nextNode=nid;m3.nextMember=mid;m3.building={stories,baysX:bx,baysY:by,storyHeights:hs.join(','),bayWidthsX:wx.join(','),bayWidthsY:wy.join(','),fixedBase:fixed,baseZ:zs[0],generateSlabs,slabThicknessMm,slabDL,slabLL,slabE_MPa,slabNu,slabGamma,slabSelfWeight,generateFootings,footingB,footingL,footingT,supportPersistenceVersion:'V1.47.2.1'};
    v1472ClearLegacySlabTransfer(m3);
    m3.slabTransferSummary=null;
    ...
    close(); alert(`V1.47 Full Building generated: ${m3.nodes.length} Nodes, ${m3.members.length} Frame Members, ${(m3.slabs||[]).length} Slabs, ${(m3.foundations||[]).length} Footings. Slabs connected as shell/plate FEM to frame.`)
...
function slabLoadPathHtmlV147(m3){
 const ss=m3.slabs||[], rr=m3.results?.slabResults||[];
 if(!ss.length)return '<div class="empty">No slabs generated. Open 3D Building and enable Generate + analyze floor slabs.</div>';
...
function v1472ClearLegacySlabTransfer(m3){
  for(const m of (m3.members||[]))for(const pat of Object.keys(m.loads3d||{})){
    const a=m.loads3d[pat];if(Array.isArray(a))m.loads3d[pat]=a.filter(x=>x?.source!=='V1.47_SLAB_TRANSFER');
  }
}
function v1472ShellProps(slab,m3){
  const b=m3.building||{};
  return {
    t:Math.max(.05,(Number(slab.thicknessMm)||Number(b.slabThicknessMm)||150)/1000),
    E:Math.max(1e6,(Number(slab.E_MPa)||Number(b.slabE_MPa)||25000)*1000), // kN/m²
    nu:Math.max(0,Math.min(.45,Number.isFinite(Number(slab.nu))?Number(slab.nu):(Number.isFinite(Number(b.slabNu))?Number(b.slabNu):.20))),
    gamma:Math.max(0,Number(slab.gamma)||Number(b.slabGamma)||24),
    selfWeight:slab.selfWeight===true || b.slabSelfWeight===true
  };
}
function v1472Shape(xi,eta){
...
function v1472BAt(coords,xi,eta){
  const sh=v1472Shape(xi,eta);let j11=0,j12=0,j21=0,j22=0;
  for(let i=0;i<4;i++){j11+=sh.dXi[i]*coords[i].x;j12+=sh.dXi[i]*coords[i].y;j21+=sh.dEt[i]*coords[i].x;j22+=sh.dEt[i]*coords[i].y}
  const det=j11*j22-j12*j21;if(Math.abs(det)<1e-10)throw new Error('Invalid/warped slab shell geometry.');
  const inv=[[j22/det,-j12/det],[-j21/det,j11/det]],dx=[],dy=[];
  for(let i=0;i<4;i++){dx[i]=inv[0][0]*sh.dXi[i]+inv[0][1]*sh.dEt[i];dy[i]=inv[1][0]*sh.dXi[i]+inv[1][1]*sh.dEt[i]}
  return {...sh,dx,dy,det:Math.abs(det)};
}
function v1472AddBtDB(K,B,D,scale){
  const DB=zeros(D.length,B[0].length);
...
function v1472AssembleSlabs(m3,K,F,imap,pat){
  const els=[];for(const slab of (m3.slabs||[])){
    slab.analysisType='Q4_SHELL_PLATE';const e=v1472Element(slab,m3,pat),dofs=[];
    for(const id of e.ids){const q=imap.get(id);if(q==null)throw new Error('Slab S'+slab.id+' references a node outside solver.');dofs.push(q*6,q*6+1,q*6+2,q*6+3,q*6+4)}
    for(let i=0;i<20;i++){F[dofs[i]]+=e.fe[i];for(let j=0;j<20;j++)K[dofs[i]][dofs[j]]+=e.Ke[i][j]}
...
function v1472CombineSlabs(terms,solved,keyName='pattern'){
  const map=new Map();for(const t of terms){const r=solved[t[keyName]],a=Number(t.factor)||0;if(!r)continue;for(const x of (r.slabResults||[])){if(!map.has(x.id))map.set(x.id,{id:x.id,story:x.story,w:0,M11:0,M22:0,M12:0,V13:0,V23:0,analysisType:'Q4_SHELL_PLATE'});const q=map.get(x.id);for(const k of ['w','M11','M22','M12','V13','V23'])q[k]+=a*Number(x[k]||0)}}return [...map.values()].sort((a,b)=>Number(a.id)-Number(b.id));
}
function solve3DV128(){
 v1472ClearLegacySlabTransfer(state.model3d||{});
 ...
 const slabElements=v1472AssembleSlabs(m3,K,F,imap,pat);
...
function v1362Combine(name,terms,solved){
  ...
  const slabResults=v1472CombineSlabs(terms,solved,'pattern');const result={U,F,displacements,reactions,memberForces,slabResults,slabFEM:{elements:slabResults.length,type:'Q4 Shell/Plate'},equilibrium,loadPattern:name,isCombination:true,terms:JSON.parse(JSON.stringify(terms))};
  try{
...
function combineLoadCaseResultsV139(name,terms,solvedCases){
  ...
  const equilibrium=equilibriumCheckV132(nodes,F,RF,imap);
  const slabResults=v1472CombineSlabs(terms,solvedCases,'caseName');
  const result={
    U,F,displacements,reactions,memberForces,slabResults,slabFEM:{elements:slabResults.length,type:'Q4 Shell/Plate'},equilibrium,
    loadPattern:name,
    isCombination:true,
    isLoadCase:false,
...
// ===== V1.47.2.1 — Easy Slab Input + Validation Fix =====
function createWebGLPhysicalRendererV14712(canvas,m3,getFocus){
 const gl=canvas.getContext('webgl',{antialias:true,alpha:false,preserveDrawingBuffer:false})||canvas.getContext('experimental-webgl');
 if(!gl)return {available:false,render:()=>{}};
 const vs=`attribute vec3 aPos;attribute vec3 aNormal;attribute vec3 aColor;uniform mat4 uMVP;varying vec3 vNormal;varying vec3 vColor;void main(){gl_Position=uMVP*vec4(aPos,1.0);vNormal=aNormal;vColor=aColor;}`;
 const fs=`precision mediump float;varying vec3 vNormal;varying vec3 vColor;uniform float uLineMode;void main(){if(uLineMode>0.5){gl_FragColor=vec4(vColor,1.0);return;}vec3 n=normalize(vNormal);vec3 l=normalize(vec3(-0.35,-0.55,0.76));float d=max(dot(n,l),0.0);float hemi=0.16+0.16*max(n.z,0.0);vec3 col=vColor*(0.52+0.42*d+hemi);col=pow(col,vec3(0.92));gl_FragColor=vec4(col,1.0);}`;
 function shader(type,src){const sh=gl.createShader(type);gl.shaderSource(sh,src);gl.compileShader(sh);if(!gl.getShaderParameter(sh,gl.COMPILE_STATUS))throw new Error(gl.getShaderInfoLog(sh)||'WebGL shader error');return sh}
 let prog;
 try{prog=gl.createProgram();gl.attachShader(prog,shader(gl.VERTEX_SHADER,vs));gl.attachShader(prog,shader(gl.FRAGMENT_SHADER,fs));gl.linkProgram(prog);if(!gl.getProgramParameter(prog,gl.LINK_STATUS))throw new Error(gl.getProgramInfoLog(prog)||'WebGL link error')}catch(e){console.warn('V1.47.1.2 WebGL unavailable',e);return {available:false,render:()=>{}}}
 const loc={p:gl.getAttribLocation(prog,'aPos'),n:gl.getAttribLocation(prog,'aNormal'),c:gl.getAttribLocation(prog,'aColor'),mvp:gl.getUniformLocation(prog,'uMVP'),line:gl.getUniformLocation(prog,'uLineMode')};
 const buf={p:gl.createBuffer(),n:gl.createBuffer(),c:gl.createBuffer(),lp:gl.createBuffer(),lc:gl.createBuffer()};
 ...
 function perspective(fovy,asp,n,f){const t=1/Math.tan(fovy/2),o=new Float32Array(16);o[0]=t/asp;o[5]=t;o[10]=(f+n)/(n-f);o[11]=-1;o[14]=2*f*n/(n-f);return o}
 function lookAt(eye,center,up){const z=norm(sub(eye,center)),x=norm(cross(up,z)),y=cross(z,x),o=new Float32Array(16);o[0]=x[0];o[1]=y[0];o[2]=z[0];o[3]=0;o[4]=x[1];o[5]=y[1];o[6]=z[1];o[7]=0;o[8]=x[2];o[9]=y[2];o[10]=z[2];o[11]=0;o[12]=-dot(x,eye);o[13]=-dot(y,eye);o[14]=-dot(z,eye);o[15]=1;return o}
 function physicalDims(mm){const A=Math.max(1e-9,Number(mm.A)||.15),Iy=Math.max(1e-12,Number(mm.Iy)||.001),Iz=Math.max(1e-12,Number(mm.Iz)||.001);let h=Math.sqrt(12*Iy/A),b=Math.sqrt(12*Iz/A);if(String(mm.memberType||'').startsWith('Column')){const d=Math.sqrt(A);b=d;h=d}return {b:Math.max(.12,Math.min(1.5,b)),h:Math.max(.12,Math.min(1.5,h))}}
 function invert4(m){const a=new Float32Array(m),o=new Float32Array(16);const b00=a[0]*a[5]-a[1]*a[4],b01=a[0]*a[6]-a[2]*a[4],b02=a[0]*a[7]-a[3]*a[4],b03=a[1]*a[6]-a[2]*a[5],b04=a[1]*a[7]-a[3]*a[5],b05=a[2]*a[7]-a[3]*a[6],b06=a[8]*a[13]-a[9]*a[12],b07=a[8]*a[14]-a[10]*a[12],b08=a[8]*a[15]-a[11]*a[12],b09=a[9]*a[14]-a[10]*a[13],b10=a[9]*a[15]-a[11]*a[13],b11=a[10]*a[15]-a[11]*a[14];let det=b00*b11-b01*b10+b02*b09+b03*b08-b04*b07+b05*b06;if(!det)return null;det=1/det;o[0]=(a[5]*b11-a[6]*b10+a[7]*b09)*det;o[1]=(-a[1]*b11+a[2]*b10-a[3]*b09)*det;o[2]=(a[13]*b05-a[14]*b04+a[15]*b03)*det;o[3]=(-a[9]*b05+a[10]*b04-a[11]*b03)*det;o[4]=(-a[4]*b11+a[6]*b08-a[7]*b07)*det;o[5]=(a[0]*b11-a[2]*b08+a[3]*b07)*det;o[6]=(-a[12]*b05+a[14]*b02-a[15]*b01)*det;o[7]=(a[8]*b05-a[10]*b02+a[11]*b01)*det;o[8]=(a[4]*b10-a[5]*b08+a[7]*b06)*det;o[9]=(-a[0]*b10+a[1]*b08-a[3]*b06)*det;o[10]=(a[12]*b04-a[13]*b02+a[15]*b00)*det;o[11]=(-a[8]*b04+a[9]*b02-a[11]*b00)*det;o[12]=(-a[4]*b09+a[5]*b07-a[6]*b06)*det;o[13]=(a[0]*b09-a[1]*b07+a[2]*b06)*det;o[14]=(-a[12]*b03+a[13]*b01-a[14]*b00)*det;o[15]=(a[8]*b03-a[9]*b01+a[10]*b00)*det;return o}
 function mv4(m,v){return [m[0]*v[0]+m[4]*v[1]+m[8]*v[2]+m[12]*v[3],m[1]*v[0]+m[5]*v[1]+m[9]*v[2]+m[13]*v[3],m[2]*v[0]+m[6]*v[1]+m[10]*v[2]+m[14]*v[3],m[3]*v[0]+m[7]*v[1]+m[11]*v[2]+m[15]*v[3]]}
 function cameraMVP(aspect){const all=m3.nodes||[];let span=10;if(all.length){const xs=all.map(n=>n.x),ys=all.map(n=>n.y),zs=all.map(n=>n.z);span=Math.max(1,Math.max(...xs)-Math.min(...xs),Math.max(...ys)-Math.min(...ys),Math.max(...zs)-Math.min(...zs))}const target=[Number(m3.view.cx)||0,Number(m3.view.cy)||0,Number(m3.view.cz)||0],yaw=(Number(m3.view.yaw)||-35)*Math.PI/180,pitch=(Number(m3.view.pitch)||24)*Math.PI/180,zoom=Math.max(5,Number(m3.view.scale)||34),dist=Math.max(span*.75,span*2.35*(34/zoom));const eye=[target[0]+dist*Math.cos(pitch)*Math.cos(yaw),target[1]+dist*Math.cos(pitch)*Math.sin(yaw),target[2]+dist*Math.sin(pitch)],P=perspective(42*Math.PI/180,aspect,Math.max(.03,dist/200),dist+span*12),V=lookAt(eye,target,[0,0,1]);return {MVP:mul4(P,V),eye,target,span,dist}}
 function memberOBBV14713(mm){const a0=m3.nodes.find(n=>n.id==mm.i),b0=m3.nodes.find(n=>n.id==mm.j);if(!a0||!b0)return null;const dim=physicalDims(mm),isCol=String(mm.memberType||'').startsWith('Column')||Math.abs(b0.z-a0.z)>Math.hypot(b0.x-a0.x,b0.y-a0.y)*1.5;let a=[a0.x,a0.y,a0.z],b=[b0.x,b0.y,b0.z],e1=norm(sub(b,a)),e2,e3;if(Math.abs(e1[2])>.92){e2=[1,0,0];e3=[0,1,0]}else{e2=norm(cross([0,0,1],e1));e3=norm(cross(e1,e2))}if(!isCol){const off=[0,0,-dim.h/2];a=add(a,off);b=add(b,off)}const center=mul(add(a,b),.5),L=Math.hypot(b[0]-a[0],b[1]-a[1],b[2]-a[2]);return {center,axes:[e1,e2,e3],ext:[L/2,dim.b/2,dim.h/2],member:mm}}
 function rayOBB(origin,dir,obb){const rel=sub(origin,obb.center),o=obb.axes.map(ax=>dot(rel,ax)),d=obb.axes.map(ax=>dot(dir,ax));let tmin=-Infinity,tmax=Infinity;for(let k=0;k<3;k++){const e=obb.ext[k];if(Math.abs(d[k])<1e-10){if(o[k]<-e||o[k]>e)return null;continue}let t1=(-e-o[k])/d[k],t2=(e-o[k])/d[k];if(t1>t2)[t1,t2]=[t2,t1];tmin=Math.max(tmin,t1);tmax=Math.min(tmax,t2);if(tmin>tmax)return null}if(tmax<0)return null;return tmin>=0?tmin:tmax}
 function pick(clientX,clientY){const rect=canvas.getBoundingClientRect();if(!(rect.width>0&&rect.height>0))return null;const ndcX=((clientX-rect.left)/rect.width)*2-1,ndcY=1-((clientY-rect.top)/rect.height)*2,cam=cameraMVP(rect.width/rect.height),inv=invert4(cam.MVP);if(!inv)return null;const unproject=z=>{const q=mv4(inv,[ndcX,ndcY,z,1]),w=Math.abs(q[3])>1e-12?q[3]:1;return [q[0]/w,q[1]/w,q[2]/w]},near=unproject(-1),far=unproject(1),dir=norm(sub(far,near));let best=null,bestT=Infinity;for(const mm of (m3.members||[])){const obb=memberOBBV14713(mm);if(!obb)continue;const t=rayOBB(near,dir,obb);if(t!=null&&t<bestT){bestT=t;best=mm}}return best}
 function render(){
  const rect=canvas.getBoundingClientRect(),d=Math.min(2,window.devicePixelRatio||1),W=Math.max(2,Math.round(rect.width*d)),H=Math.max(2,Math.round(rect.height*d));if(canvas.width!==W||canvas.height!==H){canvas.width=W;canvas.height=H}gl.viewport(0,0,W,H);
  gl.enable(gl.DEPTH_TEST);gl.depthFunc(gl.LEQUAL);gl.enable(gl.CULL_FACE);gl.cullFace(gl.BACK);gl.clearColor(0.965,0.975,0.985,1);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);
  const ps=[],ns=[],cs=[],ls=[],lcs=[];const tri=(a,b,c,col)=>{const n=norm(cross(sub(b,a),sub(c,a)));for(const p of [a,b,c]){ps.push(...p);ns.push(...n);cs.push(...col)}};const quad=(a,b,c,d,col)=>{tri(a,b,c,col);tri(a,c,d,col)};const line=(a,b,col)=>{ls.push(...a,...b);lcs.push(...col,...col)};
  const box=(v,col,edge=[.20,.23,.27])=>{const [p0,p1,p2,p3,p4,p5,p6,p7]=v;quad(p0,p3,p2,p1,col);quad(p4,p5,p6,p7,col);quad(p0,p1,p5,p4,col);quad(p1,p2,p6,p5,col);quad(p2,p3,p7,p6,col);quad(p3,p0,p4,p7,col);for(const [i,j] of [[0,1],[1,2],[2,3],[3,0],[4,5],[5,6],[6,7],[7,4],[0,4],[1,5],[2,6],[3,7]])line(v[i],v[j],edge)};
  const prism=(a0,b0,w,h,col,highlight=false,isBeam=false)=>{let a=[a0.x,a0.y,a0.z],b=[b0.x,b0.y,b0.z],e1=norm(sub(b,a)),e2,e3;if(Math.abs(e1[2])>.92){e2=[1,0,0];e3=[0,1,0]}else{e2=norm(cross([0,0,1],e1));e3=norm(cross(e1,e2))}if(isBeam){const off=[0,0,-h/2];a=add(a,off);b=add(b,off)}const c1=add(mul(e2,w/2),mul(e3,h/2)),c2=add(mul(e2,-w/2),mul(e3,h/2)),c3=add(mul(e2,-w/2),mul(e3,-h/2)),c4=add(mul(e2,w/2),mul(e3,-h/2));const v=[add(a,c1),add(a,c2),add(a,c3),add(a,c4),add(b,c1),add(b,c2),add(b,c3),add(b,c4)];box(v,highlight?[.82,.18,.16]:col,highlight?[.45,.05,.04]:[.24,.25,.27])};
  const footCol=[.48,.49,.48],slabCol=[.72,.72,.69],beamCol=[.58,.59,.58],colCol=[.64,.65,.63];
  for(const f of (m3.foundations||[])){const n=m3.nodes.find(x=>x.id===f.nodeId);if(!n)continue;const B=Number(f.B)||1.8,L=Number(f.L)||1.8,t=Number(f.t)||.45,x0=n.x-B/2,x1=n.x+B/2,y0=n.y-L/2,y1=n.y+L/2,z0=n.z-t,z1=n.z;box([[x0,y0,z0],[x1,y0,z0],[x1,y1,z0],[x0,y1,z0],[x0,y0,z1],[x1,y0,z1],[x1,y1,z1],[x0,y1,z1]],footCol)}
  for(const slab of (m3.slabs||[])){const q=(slab.nodes||[]).map(id=>m3.nodes.find(n=>n.id===id));if(q.length!==4||q.some(x=>!x))continue;const t=(Number(slab.thicknessMm)||150)/1000,top=q.map(n=>[n.x,n.y,n.z]),bot=q.map(n=>[n.x,n.y,n.z-t]);const v=[bot[0],bot[1],bot[2],bot[3],top[0],top[1],top[2],top[3]];box(v,slabCol,[.38,.39,.38])}
  const focus=getFocus?.();for(const mm of (m3.members||[])){const a=m3.nodes.find(n=>n.id==mm.i),b=m3.nodes.find(n=>n.id==mm.j);if(!a||!b)continue;const dim=physicalDims(mm),isCol=String(mm.memberType||'').startsWith('Column')||Math.abs(b.z-a.z)>Math.hypot(b.x-a.x,b.y-a.y)*1.5,hit=focus?.type==='member'&&focus.id===mm.id;prism(a,b,dim.b,dim.h,isCol?colCol:beamCol,hit,!isCol)}
  // ground/grid plane
  if(m3.nodes?.length){const xs=m3.nodes.map(n=>n.x),ys=m3.nodes.map(n=>n.y),zbase=Math.min(...m3.nodes.map(n=>n.z))-Math.max(.05,...(m3.foundations||[]).map(f=>Number(f.t)||.45));const minx=Math.min(...xs)-2,maxx=Math.max(...xs)+2,miny=Math.min(...ys)-2,maxy=Math.max(...ys)+2,step=1;for(let x=Math.floor(minx);x<=Math.ceil(maxx);x+=step)line([x,miny,zbase-.01],[x,maxy,zbase-.01],[.78,.83,.88]);for(let y=Math.floor(miny);y<=Math.ceil(maxy);y+=step)line([minx,y,zbase-.01],[maxx,y,zbase-.01],[.78,.83,.88])}
  const {MVP}=cameraMVP(W/H);
  gl.useProgram(prog);gl.uniformMatrix4fv(loc.mvp,false,MVP);
  if(ps.length){gl.bindBuffer(gl.ARRAY_BUFFER,buf.p);gl.bufferData(gl.ARRAY_BUFFER,new Float32Array(ps),gl.DYNAMIC_DRAW);gl.enableVertexAttribArray(loc.p);gl.vertexAttribPointer(loc.p,3,gl.FLOAT,false,0,0);gl.bindBuffer(gl.ARRAY_BUFFER,buf.n);gl.bufferData(gl.ARRAY_BUFFER,new Float32Array(ns),gl.DYNAMIC_DRAW);gl.enableVertexAttribArray(loc.n);gl.vertexAttribPointer(loc.n,3,gl.FLOAT,false,0,0);gl.bindBuffer(gl.ARRAY_BUFFER,buf.c);gl.bufferData(gl.ARRAY_BUFFER,new Float32Array(cs),gl.DYNAMIC_DRAW);gl.enableVertexAttribArray(loc.c);gl.vertexAttribPointer(loc.c,3,gl.FLOAT,false,0,0);gl.uniform1f(loc.line,0);gl.drawArrays(gl.TRIANGLES,0,ps.length/3)}
  if(ls.length){gl.disable(gl.CULL_FACE);gl.bindBuffer(gl.ARRAY_BUFFER,buf.lp);gl.bufferData(gl.ARRAY_BUFFER,new Float32Array(ls),gl.DYNAMIC_DRAW);gl.enableVertexAttribArray(loc.p);gl.vertexAttribPointer(loc.p,3,gl.FLOAT,false,0,0);gl.disableVertexAttribArray(loc.n);gl.vertexAttrib3f(loc.n,0,0,1);gl.bindBuffer(gl.ARRAY_BUFFER,buf.lc);gl.bufferData(gl.ARRAY_BUFFER,new Float32Array(lcs),gl.DYNAMIC_DRAW);gl.enableVertexAttribArray(loc.c);gl.vertexAttribPointer(loc.c,3,gl.FLOAT,false,0,0);gl.uniform1f(loc.line,1);gl.lineWidth(1);gl.drawArrays(gl.LINES,0,ls.length/3);gl.enable(gl.CULL_FACE)}
 }
 return {available:true,render,pick};
}
...
function initIntegrated3DV128(host){
 state.model3d ||= {nodes:[],members:[],nextNode:1,nextMember:1,view:{yaw:-35,pitch:24,scale:34}};state.model3d.view ||= {yaw:-35,pitch:24,scale:34};const m3=ensure3DLoadSystemV131(),c=host.querySelector('#v128Canvas'),glc=host.querySelector('#v14712GLCanvas'),cx=c.getContext('2d');let drag=null,view='model',tab='summary',focusTarget=null,diagramScale=1,showDiagramValues=true,diagramScope='selected',showLocalAxes=false,dragMoved=false,physicalMode=true;m3.showLoads=false;m3.view.cx??=0;m3.view.cy??=0;m3.view.cz??=0;const webglPhysical=createWebGLPhysicalRendererV14712(glc,m3,()=>focusTarget);
 ...
 function physicalDimsV1471(mm){const A=Math.max(1e-9,Number(mm.A)||.15),Iy=Math.max(1e-12,Number(mm.Iy)||.001),Iz=Math.max(1e-12,Number(mm.Iz)||.001);let h=Math.sqrt(12*Iy/A),b=Math.sqrt(12*Iz/A);if(String(mm.memberType||'').startsWith('Column')){const d=Math.sqrt(A);b=d;h=d}return {b:Math.max(.12,Math.min(1.5,b)),h:Math.max(.12,Math.min(1.5,h))}}
 function physicalMemberV1471(mm,a,bn,r,def){const A=proj(a,r.width,r.height,def),B=proj(bn,r.width,r.height,def),dim=physicalDimsV1471(mm),isCol=String(mm.memberType||'').startsWith('Column'),w=Math.max(5,(isCol?dim.b:dim.h)*m3.view.scale),dx=B.x-A.x,dy=B.y-A.y,L=Math.hypot(dx,dy)||1,nx=-dy/L,ny=dx/L,hw=w/2,pts=[{x:A.x+nx*hw,y:A.y+ny*hw},{x:B.x+nx*hw,y:B.y+ny*hw},{x:B.x-nx*hw,y:B.y-ny*hw},{x:A.x-nx*hw,y:A.y-ny*hw}];cx.save();cx.fillStyle=(focusTarget?.type==='member'&&focusTarget.id===mm.id)?'rgba(220,38,38,.82)':(isCol?'rgba(107,114,128,.88)':'rgba(75,85,99,.88)');cx.strokeStyle=(focusTarget?.type==='member'&&focusTarget.id===mm.id)?'#991b1b':'#374151';cx.lineWidth=1.2;cx.beginPath();pts.forEach((p,i)=>i?cx.lineTo(p.x,p.y):cx.moveTo(p.x,p.y));cx.closePath();cx.fill();cx.stroke();cx.restore()}
 function draw(){const r=c.getBoundingClientRect(),d=devicePixelRatio||1;c.width=Math.max(1,r.width*d);c.height=Math.max(1,r.height*d);cx.setTransform(d,0,0,d,0,0);cx.clearRect(0,0,r.width,r.height);if(physicalMode&&view==='model'&&webglPhysical.available){glc.style.display='block';c.classList.add('v14712-overlay');webglPhysical.render();drawAxisTriad();drawLegend();return}else{glc.style.display='none';c.classList.remove('v14712-overlay')}cx.fillStyle='#f8fafc';cx.fillRect(0,0,r.width,r.height);const grd=cx.createRadialGradient(r.width*.5,r.height*.42,20,r.width*.5,r.height*.45,Math.max(r.width,r.height)*.65);grd.addColorStop(0,'rgba(255,255,255,.85)');grd.addColorStop(1,'rgba(226,232,240,.35)');cx.fillStyle=grd;cx.fillRect(0,0,r.width,r.height);cx.strokeStyle='rgba(148,163,184,.20)';for(let k=-12;k<=12;k++){cx.beginPath();cx.moveTo(0,r.height/2+k*25);cx.lineTo(r.width,r.height/2+k*25);cx.stroke();cx.beginPath();cx.moveTo(r.width/2+k*25,0);cx.lineTo(r.width/2+k*25,r.height);cx.stroke()}
// V1.47.1 True Physical Building Model
if(physicalMode){
 for(const f of (m3.foundations||[])){const n=m3.nodes.find(x=>x.id===f.nodeId);if(!n)continue;const B=Number(f.B)||1.8,L=Number(f.L)||1.8,t=Number(f.t)||.45,z0=n.z-t,z1=n.z,top=[{x:n.x-B/2,y:n.y-L/2,z:z1},{x:n.x+B/2,y:n.y-L/2,z:z1},{x:n.x+B/2,y:n.y+L/2,z:z1},{x:n.x-B/2,y:n.y+L/2,z:z1}],bot=top.map(q=>({...q,z:z0})),T=top.map(q=>proj(q,r.width,r.height,false)),D=bot.map(q=>proj(q,r.width,r.height,false));cx.save();cx.shadowColor='rgba(15,23,42,.18)';cx.shadowBlur=7;cx.shadowOffsetX=4;cx.shadowOffsetY=6;cx.fillStyle='rgba(151,145,135,.96)';cx.strokeStyle='rgba(87,83,78,.75)';for(let i=0;i<4;i++){const j=(i+1)%4;cx.beginPath();cx.moveTo(T[i].x,T[i].y);cx.lineTo(T[j].x,T[j].y);cx.lineTo(D[j].x,D[j].y);cx.lineTo(D[i].x,D[i].y);cx.closePath();cx.fill();cx.stroke()}cx.fillStyle='rgba(194,188,177,.98)';cx.beginPath();T.forEach((p,i)=>i?cx.lineTo(p.x,p.y):cx.moveTo(p.x,p.y));cx.closePath();cx.fill();cx.stroke();cx.restore()}
 for(const slab of (m3.slabs||[])){const ns=(slab.nodes||[]).map(id=>m3.nodes.find(n=>n.id===id));if(ns.length!==4||ns.some(x=>!x))continue;const th=(Number(slab.thicknessMm)||150)/1000,top=ns.map(n=>({x:n.x,y:n.y,z:n.z})),bot=ns.map(n=>({x:n.x,y:n.y,z:n.z-th})),T=top.map(n=>proj(n,r.width,r.height,false)),D=bot.map(n=>proj(n,r.width,r.height,false));cx.save();cx.shadowColor='rgba(15,23,42,.10)';cx.shadowBlur=4;cx.shadowOffsetY=3;cx.fillStyle='rgba(174,168,157,.82)';cx.strokeStyle='rgba(100,96,88,.62)';for(let i=0;i<4;i++){const j=(i+1)%4;cx.beginPath();cx.moveTo(T[i].x,T[i].y);cx.lineTo(T[j].x,T[j].y);cx.lineTo(D[j].x,D[j].y);cx.lineTo(D[i].x,D[i].y);cx.closePath();cx.fill();cx.stroke()}cx.fillStyle='rgba(211,205,193,.90)';cx.beginPath();T.forEach((p,i)=>i?cx.lineTo(p.x,p.y):cx.moveTo(p.x,p.y));cx.closePath();cx.fill();cx.stroke();cx.restore()}
}
const def=view==='deformed';if(def&&m3.results){cx.save();cx.setLineDash([6,5]);cx.strokeStyle='#94a3b8';for(const mm of m3.members){const a=m3.nodes.find(n=>n.id==mm.i),b=m3.nodes.find(n=>n.id==mm.j);if(!a||!b)continue;const A=proj(a,r.width,r.height),B=proj(b,r.width,r.height);cx.beginPath();cx.moveTo(A.x,A.y);cx.lineTo(B.x,B.y);cx.stroke()}cx.restore()}
for(const mm of m3.members){const a=m3.nodes.find(n=>n.id==mm.i),b=m3.nodes.find(n=>n.id==mm.j);if(!a||!b)continue;if(physicalMode&&view==='model'){physicalMemberV1471(mm,a,b,r,false);continue}cx.strokeStyle=(focusTarget?.type==='member'&&focusTarget.id===mm.id)?'#dc2626':(def?'#2563eb':(String(mm.memberType||'').startsWith('Column')?'#f59e0b':'#0f766e'));cx.lineWidth=(focusTarget?.type==='member'&&focusTarget.id===mm.id)?6:3;const A=proj(a,r.width,r.height,def),B=proj(b,r.width,r.height,def);cx.beginPath();cx.moveTo(A.x,A.y);cx.lineTo(B.x,B.y);cx.stroke();cx.fillStyle='#334155';cx.font='11px Arial';cx.fillText('M'+mm.id,(A.x+B.x)/2+5,(A.y+B.y)/2-5)}
if(!physicalMode||view!=='model'){for(const n of m3.nodes){const q=proj(n,r.width,r.height,def),hit=focusTarget?.type==='node'&&focusTarget.id===n.id;cx.fillStyle=hit?'#dc2626':'#1d4ed8';cx.beginPath();cx.arc(q.x,q.y,hit?9:5,0,Math.PI*2);cx.fill();cx.fillStyle='#111827';cx.fillText(String(n.id),q.x+7,q.y-7)}}
drawMemberLoadsV131();drawMemberLocalAxes();drawForceDiagrams();drawAxisTriad();drawLegend()}

 function drawMemberLoadsV131(){
  if(physicalMode||!m3.showLoads)return;const pat=m3.activeLoadPattern||'DL',r=c.getBoundingClientRect();
