...
function frame3dCenterV127(){
 ...
 function draw(){const r=c.getBoundingClientRect(),d=devicePixelRatio||1;c.width=Math.max(1,r.width*d);c.height=Math.max(1,r.height*d);cx.setTransform(d,0,0,d,0,0);cx.clearRect(0,0,r.width,r.height);cx.lineWidth=1;cx.strokeStyle='#d7deea';for(let k=-8;k<=8;k++){cx.beginPath();cx.moveTo(0,r.height/2+k*25);cx.lineTo(r.width,r.height/2+k*25);cx.stroke();cx.beginPath();cx.moveTo(r.width/2+k*25,0);cx.lineTo(r.width/2+k*25,r.height);cx.stroke()}cx.lineWidth=3;cx.strokeStyle='#e18a23';for(const mm of m3.members){const a=m3.nodes.find(n=>n.id==mm.i),b=m3.nodes.find(n=>n.id==mm.j);if(!a||!b)continue;const A=proj(a,r.width,r.height),B=proj(b,r.width,r.height);cx.beginPath();cx.moveTo(A.x,A.y);cx.lineTo(B.x,B.y);cx.stroke();cx.fillStyle='#243b63';cx.fillText('M'+mm.id,(A.x+B.x)/2+5,(A.y+B.y)/2-5)}for(const n of m3.nodes){const q=proj(n,r.width,r.height);cx.fillStyle='#2457a6';cx.beginPath();cx.arc(q.x,q.y,5,0,Math.PI*2);cx.fill();cx.fillStyle='#111827';cx.fillText(String(n.id),q.x+7,q.y-7);const L=n.load||{};const parts=[];for(const k of ['fx','fy','fz'])if(Number(L[k]))parts.push(k.toUpperCase()+' '+Number(L[k])+' kN');for(const k of ['mx','my','mz'])if(Number(L[k]))parts.push(k[0].toUpperCase()+k[1]+' '+Number(L[k])+' kN·m');if(parts.length){cx.save();cx.strokeStyle='#dc2626';cx.fillStyle='#dc2626';cx.lineWidth=2;cx.beginPath();cx.moveTo(q.x+34,q.y-28);cx.lineTo(q.x+7,q.y-6);cx.stroke();cx.beginPath();cx.moveTo(q.x+7,q.y-6);cx.lineTo(q.x+14,q.y-7);cx.lineTo(q.x+10,q.y-1);cx.closePath();cx.fill();cx.font='bold 11px Arial';parts.slice(0,3).forEach((t,i)=>cx.fillText(t,q.x+38,q.y-30+i*14));if(parts.length>3)cx.fillText(parts.slice(3).join(' • '),q.x+38,q.y+14);cx.restore()}}}
 ...
 wrap.querySelector('#f3Validate').onclick=()=>{const bad=m3.members.filter(mm=>!m3.nodes.some(n=>n.id===mm.i)||!m3.nodes.some(n=>n.id===mm.j));const fixed=m3.nodes.reduce((s,n)=>s+Object.values(n.restraints||{}).filter(Boolean).length,0);if(bad.length)return alert('Invalid member connectivity: '+bad.map(x=>'M'+x.id).join(', '));if(!m3.nodes.length||!m3.members.length)return alert('Add 3D Nodes and Members first.');if(!fixed)return alert('3D model has no restrained DOF. Add supports/restraints.');toast(`V1.27 3D model valid • ${m3.nodes.length} Nodes • ${m3.members.length} Members • ${fixed} restrained DOF`)};
...
function solve3DV128(){
 ...
 const nodes=[...m3.nodes].sort((a,b)=>a.id-b.id),imap=new Map(nodes.map((n,i)=>[n.id,i])),nd=nodes.length*6,K=zeros(nd,nd),F=Array(nd).fill(0),elements=[];
 for(const n of nodes){const q=imap.get(n.id)*6,l=n.load||{};['fx','fy','fz','mx','my','mz'].forEach((key,i)=>F[q+i]=+l[key]||0)}
 for(const m of m3.members){const ni=m3.nodes.find(n=>n.id==m.i),nj=m3.nodes.find(n=>n.id==m.j);if(!ni||!nj)throw new Error('Invalid connectivity at M'+m.id);const L=Math.hypot(nj.x-ni.x,nj.y-ni.y,nj.z-ni.z);if(L<1e-9)throw new Error('Zero-length M'+m.id);const T=v128T(v128Axes(ni,nj)),kl=v128Klocal(m,L),kg=matMul(transpose(T),matMul(kl,T)),dofs=[];for(const id of [m.i,m.j]){const q=imap.get(id)*6;for(let j=0;j<6;j++)dofs.push(q+j)}for(let i=0;i<12;i++)for(let j=0;j<12;j++)K[dofs[i]][dofs[j]]+=kg[i][j];elements.push({m,T,kl,dofs})}
 ...
 const memberForces=elements.map(e=>{const ug=e.dofs.map(d=>U[d]),ul=matVec(e.T,ug),fl=matVec(e.kl,ul);return{id:e.m.id,i:e.m.i,j:e.m.j,local:fl}});m3.results={U,F,K,displacements,reactions,memberForces};return m3.results
}
function integrated3DWorkspaceV128(){
 if(integrated3dActiveV128){closeIntegrated3DV128();return}integrated3dActiveV128=true;const center=document.querySelector('.center');[...center.children].forEach(x=>x.classList.add('v128-hide2d'));$('frame3dBtn').textContent='▣ 2D Frame';$('frame3dBtn').classList.add('active3d');
 const host=document.createElement('div');host.id='integrated3dV128';host.innerHTML=`<div class="v128-toolbar"><b>3D Workspace — V1.28.2 Fix</b><button id="v128Edit3d">3D Model Data</button><button id="v128Fit">Fit</button><button id="v128L">↺</button><button id="v128R">↻</button><button id="v128U">↑</button><button id="v128D">↓</button><button id="v128Analyze" class="primary">▶ Analyze 3D</button><span id="v128TopStatus">Phase 2 • 6 DOF/node</span></div><div class="result-modes"><span class="result-modes-label">3D Results:</span><button class="result-mode active" data-v128-view="model">Model</button><button class="result-mode" data-v128-view="deformed">Deformed</button><button class="result-mode" data-v128-view="axial">Axial N</button><button class="result-mode" data-v128-view="v2">Shear V2</button><button class="result-mode" data-v128-view="v3">Shear V3</button><button class="result-mode" data-v128-view="t">Torsion T</button><button class="result-mode" data-v128-view="m2">Moment M2</button><button class="result-mode" data-v128-view="m3">Moment M3</button></div><div class="v128-view"><canvas id="v128Canvas"></canvas><div id="v128Legend" class="diagram-legend" hidden></div></div><div class="results v128-results"><div class="results-togglebar"><b>3D Analysis Results</b><span id="v128SolveStatus">Not analyzed</span></div><div class="results-body"><div class="tabs"><button class="tab active" data-v128-tab="summary">Summary</button><button class="tab" data-v128-tab="disp">Displacement</button><button class="tab" data-v128-tab="react">Reactions</button><button class="tab" data-v128-tab="forces">Member End Forces</button></div><div id="v128Out" class="result-content"><div class="empty">Press Analyze 3D to solve the model.</div></div></div></div><div class="statusbar"><span>Integrated 3D workspace • 2D engine protected</span><span>Drag: Rotate • Wheel: Zoom</span></div>`;center.appendChild(host);initIntegrated3DV128(host)
}
function closeIntegrated3DV128(){if(!integrated3dActiveV128)return;integrated3dActiveV128=false;integrated3dRefreshV128=null;document.querySelector('#integrated3dV128')?.remove();document.querySelectorAll('.v128-hide2d').forEach(x=>x.classList.remove('v128-hide2d'));$('frame3dBtn').textContent='◈ 3D Frame';$('frame3dBtn').classList.remove('active3d');resize();render();updateUI();renderResults()}
function initIntegrated3DV128(host){
 state.model3d ||= {nodes:[],members:[],nextNode:1,nextMember:1,view:{yaw:-35,pitch:24,scale:34}};state.model3d.view ||= {yaw:-35,pitch:24,scale:34};const m3=state.model3d,c=host.querySelector('#v128Canvas'),cx=c.getContext('2d');let drag=null,view='model',tab='summary';
 function proj(n,W,H,deformed=false){let x=n.x,y=n.y,z=n.z;if(deformed&&m3.results){const d=m3.results.displacements.find(q=>q.id===n.id);if(d){const amp=100;x+=d.ux*amp;y+=d.uy*amp;z+=d.uz*amp}}const yaw=m3.view.yaw*Math.PI/180,p=m3.view.pitch*Math.PI/180,X=x*Math.cos(yaw)-y*Math.sin(yaw),Y=x*Math.sin(yaw)+y*Math.cos(yaw);return{x:W/2+X*m3.view.scale,y:H/2-(z*Math.cos(p)-Y*Math.sin(p))*m3.view.scale}}
 function draw(){const r=c.getBoundingClientRect(),d=devicePixelRatio||1;c.width=Math.max(1,r.width*d);c.height=Math.max(1,r.height*d);cx.setTransform(d,0,0,d,0,0);cx.clearRect(0,0,r.width,r.height);cx.fillStyle='#fff';cx.fillRect(0,0,r.width,r.height);cx.strokeStyle='#e3e9f1';for(let k=-12;k<=12;k++){cx.beginPath();cx.moveTo(0,r.height/2+k*25);cx.lineTo(r.width,r.height/2+k*25);cx.stroke();cx.beginPath();cx.moveTo(r.width/2+k*25,0);cx.lineTo(r.width/2+k*25,r.height);cx.stroke()}const def=view==='deformed';if(def&&m3.results){cx.save();cx.setLineDash([6,5]);cx.strokeStyle='#94a3b8';for(const mm of m3.members){const a=m3.nodes.find(n=>n.id==mm.i),b=m3.nodes.find(n=>n.id==mm.j);if(!a||!b)continue;const A=proj(a,r.width,r.height),B=proj(b,r.width,r.height);cx.beginPath();cx.moveTo(A.x,A.y);cx.lineTo(B.x,B.y);cx.stroke()}cx.restore()}cx.lineWidth=3;cx.strokeStyle=def?'#2563eb':'#d97706';for(const mm of m3.members){const a=m3.nodes.find(n=>n.id==mm.i),b=m3.nodes.find(n=>n.id==mm.j);if(!a||!b)continue;const A=proj(a,r.width,r.height,def),B=proj(b,r.width,r.height,def);cx.beginPath();cx.moveTo(A.x,A.y);cx.lineTo(B.x,B.y);cx.stroke();cx.fillStyle='#334155';cx.font='11px Arial';cx.fillText('M'+mm.id,(A.x+B.x)/2+5,(A.y+B.y)/2-5)}for(const n of m3.nodes){const q=proj(n,r.width,r.height,def);cx.fillStyle='#1d4ed8';cx.beginPath();cx.arc(q.x,q.y,5,0,Math.PI*2);cx.fill();cx.fillStyle='#111827';cx.fillText(String(n.id),q.x+7,q.y-7);const L=n.load||{};const parts=[];for(const k of ['fx','fy','fz'])if(Number(L[k]))parts.push(k.toUpperCase()+' '+Number(L[k])+' kN');for(const k of ['mx','my','mz'])if(Number(L[k]))parts.push(k[0].toUpperCase()+k[1]+' '+Number(L[k])+' kN·m');if(parts.length){cx.save();cx.strokeStyle='#dc2626';cx.fillStyle='#dc2626';cx.lineWidth=2;cx.beginPath();cx.moveTo(q.x+34,q.y-28);cx.lineTo(q.x+7,q.y-6);cx.stroke();cx.beginPath();cx.moveTo(q.x+7,q.y-6);cx.lineTo(q.x+14,q.y-7);cx.lineTo(q.x+10,q.y-1);cx.closePath();cx.fill();cx.font='bold 11px Arial';parts.slice(0,3).forEach((t,i)=>cx.fillText(t,q.x+38,q.y-30+i*14));if(parts.length>3)cx.fillText(parts.slice(3).join(' • '),q.x+38,q.y+14);cx.restore()}}drawLegend()}
 function drawLegend(){const leg=host.querySelector('#v128Legend');if(!m3.results||view==='model'||view==='deformed'){leg.hidden=true;return}const map={axial:[0,6,'Axial N','kN'],v2:[1,7,'Shear V2','kN'],v3:[2,8,'Shear V3','kN'],t:[3,9,'Torsion T','kN·m'],m2:[4,10,'Moment M2','kN·m'],m3:[5,11,'Moment M3','kN·m']},q=map[view],vals=m3.results.memberForces.flatMap(f=>[f.local[q[0]],-f.local[q[1]]]);leg.hidden=false;leg.innerHTML='<b>'+q[2]+'</b><div class="legend-row"><span>Min</span><strong>'+Math.min(...vals).toFixed(3)+' '+q[3]+'</strong></div><div class="legend-row"><span>Max</span><strong>'+Math.max(...vals).toFixed(3)+' '+q[3]+'</strong></div><small>Numerical range in V1.28; full diagrams planned for V1.29.</small>'}
 function fit(){if(!m3.nodes.length){m3.view.scale=34;draw();return}const xs=m3.nodes.map(n=>n.x),ys=m3.nodes.map(n=>n.y),zs=m3.nodes.map(n=>n.z),span=Math.max(1,Math.max(...xs)-Math.min(...xs),Math.max(...ys)-Math.min(...ys),Math.max(...zs)-Math.min(...zs));m3.view.scale=Math.max(12,Math.min(75,300/span));draw()}
 integrated3dRefreshV128=(doFit=false)=>{if(doFit)fit();else draw();renderTab()};
 function renderTab(){const out=host.querySelector('#v128Out'),res=m3.results;if(!res){out.innerHTML='<div class="empty">Press Analyze 3D to solve the model.</div>';return}if(tab==='summary'){const max=Math.max(...res.displacements.map(d=>Math.hypot(d.ux,d.uy,d.uz)));out.innerHTML='<div class="v128-summary"><div><b>Nodes</b><strong>'+m3.nodes.length+'</strong></div><div><b>Members</b><strong>'+m3.members.length+'</strong></div><div><b>Total DOF</b><strong>'+(m3.nodes.length*6)+'</strong></div><div><b>Max Translation</b><strong>'+(max*1000).toFixed(4)+' mm</strong></div></div>';return}if(tab==='disp'){out.innerHTML='<table><tr><th>Node</th><th>Ux mm</th><th>Uy mm</th><th>Uz mm</th><th>Rx</th><th>Ry</th><th>Rz</th></tr>'+res.displacements.map(d=>'<tr><td>N'+d.id+'</td><td>'+(d.ux*1000).toFixed(5)+'</td><td>'+(d.uy*1000).toFixed(5)+'</td><td>'+(d.uz*1000).toFixed(5)+'</td><td>'+d.rx.toExponential(3)+'</td><td>'+d.ry.toExponential(3)+'</td><td>'+d.rz.toExponential(3)+'</td></tr>').join('')+'</table>';return}if(tab==='react'){out.innerHTML='<table><tr><th>Node</th><th>Fx</th><th>Fy</th><th>Fz</th><th>Mx</th><th>My</th><th>Mz</th></tr>'+res.reactions.map(d=>'<tr><td>N'+d.id+'</td><td>'+d.fx.toFixed(4)+'</td><td>'+d.fy.toFixed(4)+'</td><td>'+d.fz.toFixed(4)+'</td><td>'+d.mx.toFixed(4)+'</td><td>'+d.my.toFixed(4)+'</td><td>'+d.mz.toFixed(4)+'</td></tr>').join('')+'</table>';return}out.innerHTML='<table><tr><th>M</th><th>N i</th><th>V2 i</th><th>V3 i</th><th>T i</th><th>M2 i</th><th>M3 i</th><th>N j</th><th>V2 j</th><th>V3 j</th><th>T j</th><th>M2 j</th><th>M3 j</th></tr>'+res.memberForces.map(f=>'<tr><td>M'+f.id+'</td>'+f.local.map(v=>'<td>'+v.toFixed(3)+'</td>').join('')+'</tr>').join('')+'</table>'}
 function analyze(){try{const res=solve3DV128();host.querySelector('#v128SolveStatus').textContent='Solved • '+(m3.nodes.length*6)+' DOF';renderTab();draw();toast('V1.28 3D analysis complete')}catch(e){alert(e.message)}}
 host.querySelector('#v128Edit3d').onclick=frame3dCenterV127;host.querySelector('#v128Fit').onclick=fit;host.querySelector('#v128L').onclick=()=>{m3.view.yaw-=10;draw()};host.querySelector('#v128R').onclick=()=>{m3.view.yaw+=10;draw()};host.querySelector('#v128U').onclick=()=>{m3.view.pitch=Math.min(80,m3.view.pitch+8);draw()};host.querySelector('#v128D').onclick=()=>{m3.view.pitch=Math.max(-80,m3.view.pitch-8);draw()};host.querySelector('#v128Analyze').onclick=analyze;host.querySelectorAll('[data-v128-view]').forEach(b=>b.onclick=()=>{view=b.dataset.v128View;host.querySelectorAll('[data-v128-view]').forEach(x=>x.classList.toggle('active',x===b));draw()});host.querySelectorAll('[data-v128-tab]').forEach(b=>b.onclick=()=>{tab=b.dataset.v128Tab;host.querySelectorAll('[data-v128-tab]').forEach(x=>x.classList.toggle('active',x===b));renderTab()});c.onpointerdown=e=>drag={x:e.clientX,y:e.clientY,yaw:m3.view.yaw,pitch:m3.view.pitch};c.onpointermove=e=>{if(!drag)return;m3.view.yaw=drag.yaw+(e.clientX-drag.x)*.35;m3.view.pitch=Math.max(-80,Math.min(80,drag.pitch-(e.clientY-drag.y)*.25));draw()};c.onpointerup=c.onpointerleave=()=>drag=null;c.onwheel=e=>{e.preventDefault();m3.view.scale=Math.max(5,Math.min(130,m3.view.scale*(e.deltaY>0?.9:1.1)));draw()};draw();setTimeout(fit,0)
}

$('frame3dBtn').onclick=integrated3DWorkspaceV128;

updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.28.2 Fix — selected 3D Node preserved • 3D nodal load visualization • 2D engine preserved');
})();
