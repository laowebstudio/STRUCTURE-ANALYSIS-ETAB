...
function rcBeamDesignV125(m){
 ...
 const dim=sec.dimensions||{};const bM=Number(dim.b||0),hM=Number(dim.h||0);
...
function rcBeamDetailV125(m){
 ...
 const wrap=document.createElement('div');wrap.className='eng-dialog';wrap.innerHTML=`<div class="eng-card steel-detail-v122"><div class="section-db-head"><div><h2>RC Beam Design Detail — M${m.id}</h2><small>${sec.name||m.sectionId||'Section'} • ACI CODE-318-25 • V1.25 Phase 1 Flexure</small></div><button class="ml-close">×</button></div><div class="steel-detail-grid">
...
function designCenterV120(){
 const wrap=document.createElement('div');wrap.className='eng-dialog design-modal-v120';wrap.innerHTML=`<div class="eng-card design-card-v120"><div class="section-db-head"><div><h2>◆ Design Center — V1.25</h2><small>Steel Design V1.24 preserved • RC Beam Design Phase 1 • ACI CODE-318-25 Flexure</small></div><button class="ml-close" id="d120Close">×</button></div><div id="d120Body"></div></div>`;document.body.appendChild(wrap);wrap.querySelector('#d120Close').onclick=()=>wrap.remove();wrap.onclick=e=>{if(e.target===wrap)wrap.remove()};const body=wrap.querySelector('#d120Body');
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.25 — RC Beam Design Phase 1 ready • Steel V1.24 preserved');
})();
