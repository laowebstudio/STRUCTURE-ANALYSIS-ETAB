...
function building3dCenterV130(){
  ...
  wrap.innerHTML=`<div class="eng-card v130-building-card"><div class="section-db-head"><div><h2>Full Building Generator — V1.47.1</h2><small>Physical Beam + Column + Slab + Foundation model • whole-building frame solve • 2D model protected</small></div><button class="ml-close">×</button></div>
  <div class="v130-building-grid">
   ...
   <section><h3>Stories</h3>
    ...
    <div class="engineering-note"><b>Easy Slab Setup:</b> Set thickness and floor loads only. V1.47.2 automatically creates one compatible 4-node shell/plate element per structural bay and connects it directly to the surrounding Beam/Column joints.</div>
    <div class="v130-mini"><label>Slab thickness (mm)<input id="v147SlabT" type="number" min="80" step="10" value="${Number(b.slabThicknessMm)||150}"></label><label>Concrete E (MPa)<input id="v147SlabE" type="number" min="10000" step="500" value="${Number(b.slabE_MPa)||25000}"></label></div>
    <div class="v130-mini"><label>DL superimposed (kN/m²)<input id="v147SlabDL" type="number" step="0.1" value="${Number(b.slabDL)||-1.0}"></label><label>LL (kN/m²)<input id="v147SlabLL" type="number" step="0.1" value="${Number(b.slabLL)||-2.0}"></label></div>
    <div class="v130-mini"><label>Poisson ν<input id="v147SlabNu" type="number" min="0" max="0.45" step="0.01" value="${Number.isFinite(Number(b.slabNu))?Number(b.slabNu):0.20}"></label><label>Concrete unit weight (kN/m³)<input id="v147SlabGamma" type="number" min="10" max="30" step="0.5" value="${Number(b.slabGamma)||24}"></label></div>
  ...
  <div class="engineering-note"><b>V1.47.2:</b> Floor slabs are solver-linked 4-node shell/plate finite elements (membrane + Mindlin bending/shear) sharing the same corner nodes as the frame. Area loads enter the global stiffness/load system directly; legacy tributary beam loads are removed automatically.</div></div>`;
  document.body.appendChild(wrap);
  ...
  q('v130Generate').onclick=()=>{try{
    ...
    const generateSlabs=!!q('v147Slabs').checked,slabThicknessMm=Math.max(80,+q('v147SlabT').value||150),slabDL=Number(q('v147SlabDL').value)||0,slabLL=Number(q('v147SlabLL').value)||0,slabE_MPa=Math.max(10000,Number(q('v147SlabE')?.value)||25000),slabNu=Math.max(0,Math.min(.45,Number(q('v147SlabNu')?.value)||.20)),slabGamma=Math.max(10,Number(q('v147SlabGamma')?.value)||24),slabSelfWeight=!!q('v147SlabSW')?.checked;
    if(generateSlabs){let sid=1;for(let k=1;k<=stories;k++)for(let iy=0;iy<by;iy++)for(let ix=0;ix<bx;ix++)m3.slabs.push({id:sid++,story:k,nodes:[map.get(`${ix},${iy},${k}`),map.get(`${ix+1},${iy},${k}`),map.get(`${ix+1},${iy+1},${k}`),map.get(`${ix},${iy+1},${k}`)],thicknessMm:slabThicknessMm,dl:slabDL,ll:slabLL,analysisType:'Q4_SHELL_PLATE',E_MPa:slabE_MPa,nu:slabNu,gamma:slabGamma,selfWeight:slabSelfWeight,mesh:{nx:1,ny:1,compatible:true},source:'V1.47.2_SHELL_PLATE'})}
    ...
    m3.nextNode=nid;m3.nextMember=mid;m3.building={stories,baysX:bx,baysY:by,storyHeights:hs.join(','),bayWidthsX:wx.join(','),bayWidthsY:wy.join(','),fixedBase:fixed,baseZ:zs[0],generateSlabs,slabThicknessMm,slabDL,slabLL,slabE_MPa,slabNu,slabGamma,slabSelfWeight,generateFootings,footingB,footingL,footingT,supportPersistenceVersion:'V1.47.2'};
...
function v1472Element(slab,m3,pat){
  ...
  const q0=pat==='DL'?(Number(slab.dl)||0)+(pr.selfWeight?-pr.gamma*t:0):pat==='LL'?(Number(slab.ll)||0):0;
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.47.2 — Slab Shell/Plate Analysis Engine');
})();
