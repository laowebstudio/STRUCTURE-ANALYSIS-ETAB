...
if(physicalMode){
 for(const f of (m3.foundations||[])){const n=m3.nodes.find(x=>x.id===f.nodeId);if(!n)continue;const B=Number(f.B)||1.8,L=Number(f.L)||1.8,t=Number(f.t)||.45,z0=n.z-t,z1=n.z,top=[{x:n.x-B/2,y:n.y-L/2,z:z1},{x:n.x+B/2,y:n.y-L/2,z:z1},{x:n.x+B/2,y:n.y+L/2,z:z1},{x:n.x-B/2,y:n.y+L/2,z:z1}],bot=top.map(q=>({...q,z:z0})),T=top.map(q=>proj(q,r.width,r.height,false)),D=bot.map(q=>proj(q,r.width,r.height,false));cx.save();cx.fillStyle='rgba(120,113,108,.72)';cx.strokeStyle='#57534e';for(let i=0;i<4;i++){const j=(i+1)%4;cx.beginPath();cx.moveTo(T[i].x,T[i].y);cx.lineTo(T[j].x,T[j].y);cx.lineTo(D[j].x,D[j].y);cx.lineTo(D[i].x,D[i].y);cx.closePath();cx.fill();cx.stroke()}cx.fillStyle='rgba(168,162,158,.9)';cx.beginPath();T.forEach((p,i)=>i?cx.lineTo(p.x,p.y):cx.moveTo(p.x,p.y));cx.closePath();cx.fill();cx.stroke();cx.restore()}
 for(const slab of (m3.slabs||[])){const ns=(slab.nodes||[]).map(id=>m3.nodes.find(n=>n.id===id));if(ns.length!==4||ns.some(x=>!x))continue;const th=(Number(slab.thicknessMm)||150)/1000,top=ns.map(n=>({x:n.x,y:n.y,z:n.z})),bot=ns.map(n=>({x:n.x,y:n.y,z:n.z-th})),T=top.map(n=>proj(n,r.width,r.height,false)),D=bot.map(n=>proj(n,r.width,r.height,false));cx.save();cx.fillStyle='rgba(156,163,175,.48)';cx.strokeStyle='rgba(75,85,99,.75)';for(let i=0;i<4;i++){const j=(i+1)%4;cx.beginPath();cx.moveTo(T[i].x,T[i].y);cx.lineTo(T[j].x,T[j].y);cx.lineTo(D[j].x,D[j].y);cx.lineTo(D[i].x,D[i].y);cx.closePath();cx.fill();cx.stroke()}cx.fillStyle='rgba(209,213,219,.62)';cx.beginPath();T.forEach((p,i)=>i?cx.lineTo(p.x,p.y):cx.moveTo(p.x,p.y));cx.closePath();cx.fill();cx.stroke();cx.restore()}
}
...
drawMemberLoadsV131();drawMemberLocalAxes();drawForceDiagrams();drawAxisTriad();drawLegend()}
 ...
 host.querySelector('#v1471Physical').onclick=()=>{physicalMode=true;m3.showLoads=false;host.querySelector('#v1471Physical').classList.add('primary');host.querySelector('#v1471Analysis').classList.remove('primary');host.querySelector('#v128TopStatus').textContent='V1.47.1 • TRUE PHYSICAL MODEL • Beam + Column + Slab + Foundation';draw()};host.querySelector('#v1471Analysis').onclick=()=>{physicalMode=false;host.querySelector('#v1471Analysis').classList.add('primary');host.querySelector('#v1471Physical').classList.remove('primary');host.querySelector('#v128TopStatus').textContent='V1.47.1 • ANALYSIS MODEL • Centerlines + Loads + Results';draw()};
...
updateEngineeringSelectors();migrateLoads();resize();updateUI();renderResults();updateResultModeButtons();setResultView('model',false);setTool('select');syncScaleUI();initResultsWorkspaceV113();toast('V1.47.1 — True Physical Building Model');
})();
